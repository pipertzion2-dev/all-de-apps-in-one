/**
 * Manufacturing Readiness Checker — client-side generation, <3s target.
...
(function () {
  ...
  const MODE_CONFIG = {
    hardware: {
      stepName: 'Hardware pipeline — readiness check',
      steps: ['Concept input', 'Readiness check (this preview)', 'BOM generation', 'Schematic layout', 'Cost tracking', 'Export'],
      artifactType: 'Schematic + BOM preview',
    },
    play: {
      stepName: 'Svivva Play — readiness',
      steps: ['Intent input', 'Readiness / setup match (this preview)', 'Performance modes', 'Routing & FX', 'Session export'],
      artifactType: 'Audio / setup map',
    },
    idea: {
      stepName: 'Idea Engine — readiness',
      steps: ['Idea input', 'Readiness / concept scope (this preview)', 'Stage expansion', 'Validation', 'Report export'],
      artifactType: 'Idea report',
    },
    team: {
      stepName: 'Collaboration — readiness',
      steps: ['Scenario input', 'Readiness / scope match (this preview)', 'Permissions', 'Versioning', 'Handoff'],
      artifactType: 'Setup map',
    },
    marketplace: {
      stepName: 'API Marketplace — readiness',
      steps: ['Product idea', 'Readiness / publish match (this preview)', 'API config', 'Listing', 'Distribution'],
      artifactType: 'Flow graph',
    },
    build: {
      stepName: 'B.U.I.L.D. — readiness',
      steps: ['Build input', 'Readiness check (this preview)', 'Digital twin', 'Physical flow', 'Production export'],
  ...
  function generatePreview(data) {
    ...
    if (mode === 'hardware' || mode === 'build') {
      ...
      html =
        ...
        '<p><strong>Readiness summary:</strong> Concept aligns with cost and feasibility scope. Next in Svivva: expand stages, validate assumptions, and export idea report.</p>';
    } else if (mode === 'team') {
      html =
        '<div class="preview-title">' + config.artifactType + '</div>' +
        '<p>Scenario: ' + escapeHtml(desc) + '</p>' +
        '<p><strong>Readiness:</strong> Collaboration size and cost scope matched. In full Svivva: set permissions, versioning, and handoff workflows.</p>' +
        (data.collabSize ? '<p><strong>Collaboration size:</strong> ' + escapeHtml(data.collabSize) + '</p>' : '');
    } else {
      html =
        '<div class="preview-title">' + config.artifactType + '</div>' +
        '<p>Product idea: ' + escapeHtml(desc) + '</p>' +
        '<p><strong>Readiness path:</strong> Publish match → API config → Listing → Distribution. This preview is step one.</p>' +
  ...
  function handleSubmit(e) {
    ...
    setTimeout(function () {
      ...
      if (btn) {
        btn.disabled = false;
        btn.textContent = 'Check readiness';
      }
  ...
  function init() {
    ...
    bindCtaClick('hero-cta-primary', 'hero_primary');
    bindCtaClick('hero-cta-secondary', 'hero_secondary');
    bindCtaClick('hero-cta-link', 'hero_inline_link');
    bindCtaClick('upgrade-cta', 'right_panel_upgrade');
    bindCtaClick('outcomes-cta', 'outcomes_section');

    document.querySelectorAll('.use-case-link').forEach(function (el, i) {
      el.addEventListener('click', function () {
        track('CTA click', { location: 'use_case_card', index: i, text: el.textContent.trim() });
      });
    });

    document.querySelectorAll('.seo-content a').forEach(function (el) {
      el.addEventListener('click', function () {
        track('CTA click', { location: 'seo_content', text: el.textContent.trim() });
      });
    });
  }

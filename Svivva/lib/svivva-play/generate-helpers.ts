...
export function meendApplicableStemNames(
...
function generateJazzBassLine(
  ...
  events.push({
    ...
    velocity: 90,
    startBeat,
    duration: Math.min(0.88, spanBeats * 0.4),
    channel: 1,
  });
  ...
  if (spanBeats >= 3) {
    events.push({ note: fifth, velocity: 78, startBeat: startBeat + 2, duration: 0.8, channel: 1 });
  }
...
export function generateSmartChordStems(
  ...
  const srcChords: ChordSegment[] =
    sessionChords.length > 0
      ? sessionChords
      : [
          {
            t0: 0,
  ...
  while (beat < maxBeat) {
    ...
    const nextBass = nextSrc
      ? Math.max(
          ...
          Math.min(
            50,
            24 + ChordKit.parseRoot(nextSrc.symbol.match(/^([A-Ga-g][b#♭♯]?)/)?.[1] ?? "C"),
          ),
        )
      : null;

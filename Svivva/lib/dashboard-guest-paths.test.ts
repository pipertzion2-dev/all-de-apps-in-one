...
describe("dashboard guest paths", () => {
  it("allows Burns, GSC, and Orbit SEO connect without session (admin code first)", () => {
    expect(isAdminCodeFirstPath("/dashboard/burns")).toBe(true);
    expect(isAdminCodeFirstPath("/dashboard/gsc-connect")).toBe(true);
    expect(isAdminCodeFirstPath("/dashboard/orbit/connect")).toBe(true);
    expect(isDashboardGuestPath("/dashboard/burns")).toBe(true);
    expect(isDashboardGuestPath("/dashboard/orbit/connect")).toBe(true);
  });

  ...
  switch (step.id) {
    ...
    case "seo-monitor": {
      ...
      return {
        ...
        summary:
          critical === 0
            ? `Monitor OK — ${monitor.metrics.sitemapUrls} sitemap URLs, ${monitor.alerts.length} alert(s).`
            : `${critical} critical alert(s): ${monitor.alerts
                .filter((a) => a.severity === "critical")
                .slice(0, 3)
                .map((a) => a.message)
                .join(" · ")}`,
        detail: { alerts: monitor.alerts.slice(0, 8) },
      };
    }
    case "performance-review": {
      ...
      return {
        ok: true,
        summary: `Property ${gsc.siteUrl}: ${impressions.toLocaleString()} impressions, ${clicks} clicks, ${gsc.pages.length} pages with data (28d). Zero is common early — keep weekly indexing, not daily floods.`,
        detail: {
          impressions,
          clicks,

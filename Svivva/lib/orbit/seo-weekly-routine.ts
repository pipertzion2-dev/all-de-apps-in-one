...
import { fetchGscSearchAnalytics, type GscSearchAnalyticsReport } from "@/lib/seo/gsc-search-analytics";
...
function buildSummary(tasks: WeeklyTaskResult[], gsc: GscSearchAnalyticsReport | null): string {
  const lines = [
    ...
    ...tasks.map((t) => {
      const icon =
        t.status === "done" ? "✓" : t.status === "partial" ? "◐" : t.status === "needs_credentials" ? "🔑" : "✗";
...
export async function runSeoWeeklyRoutine(opts?: {
  ...
  tasks.push(
    task(
      ...
      gsc.ok
        ? { topPages: gsc.pages.slice(0, 5).map((p) => ({ page: p.keys[0], impressions: p.impressions })) }
        : undefined,
    ),
  );
  ...
  tasks.push(
    task(
      ...
      gsc.ok
        ? `${gsc.nearPageOne.length} keywords close to page 1`
        : "Skipped — connect GSC",
      gsc.ok
        ? {
  ...
  try {
    ...
    linkMsg = heal.updated ? `Added related links on ${heal.updated} pages` : "Internal link graph healthy";

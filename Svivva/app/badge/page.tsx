...
function CopyBlock({ code, label }: { code: string; label: string }) {
  ...
  return (
    <div className="rounded-xl border border-border/50 overflow-hidden">
      ...
      <pre className="px-4 py-3 text-xs font-mono text-foreground/80 overflow-x-auto leading-relaxed whitespace-pre-wrap">{code}</pre>
    </div>
  );
}
    ...
    <div className="max-w-2xl mx-auto px-4 py-16 space-y-12">
      ...
      <div className="space-y-3">
        ...
        <p className="text-muted-foreground text-sm leading-relaxed">
          Add a "Built with Svivva" badge to your README, website, or app. Shows the world you're using the fastest AI API builder — and gives Svivva a small backlink in return.
        </p>
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="text-xs" style={{ borderColor: `${TEAL}40`, color: TEAL }}>Free</Badge>
          <Badge variant="outline" className="text-xs border-border/50 text-muted-foreground">No signup required</Badge>
          <Badge variant="outline" className="text-xs border-border/50 text-muted-foreground">SVG format</Badge>
        </div>
      </div>

      {/* Badge variants */}
      <div className="space-y-4">
        <h2 className="text-sm font-bold uppercase tracking-wide text-muted-foreground">Badge Variants</h2>
        {badges.map((b) => {
          ...
          return (
            <div key={b.label} className="rounded-2xl border border-border/50 bg-card p-5 space-y-4">
      ...
      <div className="rounded-2xl border border-border/50 bg-card p-5 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">Customize the label, message, and color using URL parameters.</p>
        <CopyBlock
          code={`${badgeUrl}?label=your+label&message=your+message&color=5BA8A0`}
          label="Custom badge URL"
        />
        <div className="text-xs text-muted-foreground space-y-1">
          <p><code className="text-foreground">label</code> — left side text (URL-encode spaces as <code>+</code>)</p>
          <p><code className="text-foreground">message</code> — right side text</p>
          <p><code className="text-foreground">color</code> — hex color without # (default: 5BA8A0)</p>
        </div>
      </div>

      {/* CTA */}
      <div className="rounded-2xl p-6 text-center space-y-3" style={{ background: `${TEAL}15`, border: `1px solid ${TEAL}30` }}>
        <p className="text-sm font-semibold">Build your own AI API in 11 minutes</p>
        <p className="text-xs text-muted-foreground">Svivva turns natural language prompts into production-ready APIs with schema enforcement, versioning, and a built-in marketplace.</p>
        <Link href="/" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold text-white" style={{ background: TEAL }}>

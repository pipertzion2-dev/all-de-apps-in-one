...
interface PatchData {
  ...
  filter: { type: string; cutoff_hz: number; resonance: number; drive?: number; key_track?: number };
  env: {
...
const MODE_CONFIG: Record<PlayMode, { label: string; shortLabel: string; icon: React.ComponentType<{ className?: string }>; description: string; qualityTier: string }> = {
  composition: { label: "Composition", shortLabel: "Comp", icon: Music, description: "Counterpoint & hocketing — interlocking canons, phased textures", qualityTier: "professional" },
  interpolation: { label: "Interpolation", shortLabel: "Interp", icon: Waves, description: "Style-transfer loops preserving rhythmic/harmonic DNA", qualityTier: "professional" },
  chords: { label: "Chord Player", shortLabel: "Chords", icon: Piano, description: "Advanced voicings, reharmonization, and comping patterns", qualityTier: "professional" },
  solo: { label: "Solo Prompt", shortLabel: "Solo", icon: GiSaxophone, description: "AI soloist with phrasing development and harmonic awareness", qualityTier: "professional" },
  patch: { label: "Patch Creator", shortLabel: "Patch", icon: Settings2, description: "Universal synth patch design from audio analysis", qualityTier: "professional" },
...
const STYLE_PRESETS: Record<PlayMode, { id: string; label: string; desc: string }[]> = {
  composition: [
    ...
    { id: "hocketed_texture", label: "Hocketed Texture", desc: "Alternating note distribution, staggered entrances" },
    ...
    { id: "rational_melody", label: "Rational Melody", desc: "Limited pitch set with counting rules" },
    { id: "counting_process", label: "Counting Process", desc: "Arithmetic-generated note structures" },
    { id: "combinatorial", label: "Combinatorial Harmony", desc: "Chord set traversal and catalog" },
  ],
  interpolation: [
    { id: "genre_transfer", label: "Genre Transfer", desc: "Transform genre while keeping DNA" },
    { id: "texture_transfer", label: "Texture Transfer", desc: "Same harmony, entirely new timbres" },
  ...
  chords: [
    ...
    { id: "sophisticated_pop", label: "Sophisticated Pop", desc: "Color chords with melodic hooks" },
  ...
  ensemble: [
    { id: "cinematic_orchestra", label: "Cinematic Orchestra", desc: "40-piece with dynamics pp-ff" },
    ...
    { id: "70s_jazz_rock", label: "70s Sophisticated Jazz-Rock", desc: "Electric piano, sax, trumpet" },
...
export default function SvivvaPlayPage() {
  ...
  const [versionHistory, setVersionHistory] = useState<{ id: string; seed: number; stemsCount: number; timestamp: number; label: string; stems: StemData[]; plan: PlanInfo | null; patch: PatchData | null }[]>([]);
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch { /* silent — will retry on play */ }
  ...
  const buildSettings = useCallback(() => {
    ...
    const base = {
      density, complexity, harmonyMode, meend, userPrompt: userPrompt || undefined,
      seed: currentSeed,
    };
    switch (mode) {
      case "chords":
      ...
      case "patch":
        return { ...base, synthFamily, macros: { brightness: macroBrightness, movement: macroMovement, bite: macroBite, space: macroSpace } };
      case "ensemble":
        return { ...base, vocalistEnabled, ensembleSize: selectedPreset === "cinematic_orchestra" ? 40 : 12 };
      default:
        return base;
    }
  ...
  const handleGenerate = useCallback(async (quality: "preview" | "full" = "preview") => {
    ...
    setPipelineStage(mode === "patch" ? "Designing patch..." : "Stage 1: Planning arrangement...");
    setStems([]);
    setPatchResult(null);
    setPlanInfo(null);
    setErrorMsg("");
    ...
    try {
      ...
      if (mode === "solo") {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          const res = await fetch("/api/svivva-play/generate", {
            ...
            body: JSON.stringify({
              ...
              quality, settings: { ...settings, seed: takeSeed },
        ...
        if (takes.length > 0) {
          ...
          const ver = { id: String(Date.now()), seed: settings.seed, stemsCount: takes[0].stems.length, timestamp: Date.now(), label: `Solo v${versionHistory.length + 1}`, stems: takes[0].stems, plan: null, patch: null };
        ...
        if (data.error) {
          ...
          setGenerationId(data.generationId);
          setPipelineStage("Complete");
          const ver = { id: data.generationId || String(Date.now()), seed: settings.seed, stemsCount: data.stems.length, timestamp: Date.now(), label: `${MODE_CONFIG[mode].label} v${versionHistory.length + 1}`, stems: data.stems, plan: data.plan || null, patch: null };
          setVersionHistory(prev => [...prev, ver]);
          setActiveVersion(versionHistory.length);
        } else if (data.patch) {
          setPatchResult(data.patch);
          setPipelineStage("Complete");
          const ver = { id: String(Date.now()), seed: settings.seed, stemsCount: 0, timestamp: Date.now(), label: `Patch v${versionHistory.length + 1}`, stems: [], plan: null, patch: data.patch };
          setVersionHistory(prev => [...prev, ver]);
          setActiveVersion(versionHistory.length);
        }
      }
  ...
  const handleRegenerateStem = useCallback(async (stemName: string) => {
  ...
  const handleVersionSwitch = useCallback((vi: number) => {
    setActiveVersion(vi);
  ...
  const toggleStemLock = useCallback((stemName: string) => {
    setLockedStems(prev => {
      ...
      if (next.has(stemName)) { next.delete(stemName); } else { next.add(stemName); }
      return next;
    });
  }, []);

  const handleChordEdit = useCallback((index: number, newSymbol: string) => {
  ...
  const handleExport = useCallback(async (format: "json" | "midi" | "patch" = "json") => {
    if (!sessionId) return;
    try {
  ...
  const loadStemsIntoEngine = useCallback(async (currentStems: StemData[], bpm: number) => {
    try {
  ...
  useEffect(() => {
    ...
    const engine = getSoundEngine();
    engine.applySoloState(stems.map(s => ({ name: s.name, soloed: s.soloed, muted: s.muted, gainDb: s.gainDb })));
  }, [stems, engineReady]);
  ...
  const clearAudio = useCallback(() => {
    setAudioFile(null); setAudioUrl(null); setAudioName(""); setAnalysis(null);
    setManualTempo(null); setManualKey(null);
    setStems([]); setPatchResult(null); setPlanInfo(null); setSessionId(null);
    setIsPlaying(false); setErrorMsg(""); setEngineReady(false); setPlaybackPos(0); setPlaybackDuration(0);
    stopPositionTracking();
    try { getSoundEngine().dispose(); } catch {}
  ...
  const handleGenerateNeuralPrompt = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/svivva-play/neural-prompt", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          analysis: { bpm: manualTempo ?? analysis.bpm, key: manualKey ?? analysis.key, timeSignature: analysis.timeSignature, chords: analysis.chords?.slice(0, 16), sections: analysis.sections },
          stems: stems.map(s => ({ name: s.name, role: s.role, instrumentHint: s.instrumentHint, midiEvents: (s.midiEvents || []).slice(0, 20) })),
          mode,
          style: selectedPreset,
          config: { genre: neuralGenre || undefined, mood: neuralMood || undefined, energy: neuralEnergy, quality: neuralQuality },
        }),
      });
      const data = await res.json();
  ...
  useEffect(() => {
    ...
    setStems([]); setPatchResult(null); setPlanInfo(null); setPipelineStage(""); setErrorMsg("");
    setIsPlaying(false);
    setPlaybackPos(0);
    ...
    setNeuralPromptResult(null);
    stopPositionTracking();
    try { getSoundEngine().stop(); getSoundEngine().dispose(); } catch {}
  ...
  const handleLocalCompositionGenerate = useCallback(() => {
    ...
    try {
      ...
      const voices = reichType === "counterpoint"
        ? composeCounterpoint({ durationSec: reichDuration, bpm, scale, style: reichStyle, seed: currentSeed })
        ...
        return (
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-4">
              <RadioOption label="Match Harmony" checked={harmonyMode === "match"} onChange={() => setHarmonyMode("match")} />
              <RadioOption label="Reharmonize" checked={harmonyMode === "reharmonize"} onChange={() => setHarmonyMode("reharmonize")} />
            </div>
            <CheckboxOption label="Indian Meend (continuous pitch bend)" checked={meend} onChange={setMeend} />
          </div>
        );
      case "chords":
        return (
          <div className="space-y-3">
            ...
            <SliderControl label="Root Movement (stable \u2194 adventurous)" value={rootMovement} onChange={setRootMovement} />
            <div>
              <label className="text-[10px] font-medium text-gray-500 uppercase tracking-wider">Voice Leading</label>
              <div className="flex items-center gap-3 mt-1">
                {(["smooth", "moderate", "jumpy"] as const).map(v => (
                  <RadioOption key={v} label={v.charAt(0).toUpperCase() + v.slice(1)} checked={voiceLeading === v} onChange={() => setVoiceLeading(v)} />
                ))}
              </div>
            </div>
            <div>
              <label className="text-[10px] font-medium text-gray-500 uppercase tracking-wider">Comping Pattern</label>
              <div className="grid grid-cols-2 gap-1.5 mt-1">
                {["sustained_pads", "rhythmic_stabs", "arpeggiated", "guitar_like"].map(p => (
                  <button key={p} onClick={() => setCompingPattern(p)}
            ...
            <CheckboxOption label="Meend (usually off for chords)" checked={meend} onChange={setMeend} />
          </div>
        );
      case "solo":
        return (
          <div className="space-y-3">
            <div>
              <label className="text-[10px] font-medium text-gray-500 uppercase tracking-wider">Solo Type</label>
              <div className="flex items-center gap-3 mt-1">
                {[{v:"instrument",l:"Instrument"},{v:"vocal_scat",l:"Vocal Scat"},{v:"harmony_solo",l:"Harmony Solo"}].map(t => (
                  <RadioOption key={t.v} label={t.l} checked={soloType === t.v} onChange={() => setSoloType(t.v)} />
                ))}
              </div>
              {soloType === "vocal_scat" && (
                <div className="flex items-center gap-1.5 mt-1.5">
                  <AlertTriangle className="w-3 h-3 text-amber-500" />
                  <span className="text-[10px] text-amber-600">Audio preview for vocal scat is BETA quality</span>
                </div>
              )}
            </div>
            <SliderControl label="Risk (inside \u2194 outside)" value={risk} onChange={setRisk} />
            <SliderControl label="Density" value={density} onChange={setDensity} />
            <CheckboxOption label="Call-and-Response" checked={callResponse} onChange={setCallResponse} />
            <CheckboxOption label="Meend (continuous pitch bend)" checked={meend} onChange={setMeend} />
          </div>
        );
      case "interpolation":
        return (
          <div className="space-y-3">
            <SliderControl label="Style Strength (0=original, 100=full transform)" value={styleStrength} onChange={setStyleStrength} />
            <CheckboxOption label="Keep Original Harmony" checked={keepHarmony} onChange={setKeepHarmony} />
            <SliderControl label="Density" value={density} onChange={setDensity} />
          </div>
        );
      case "patch":
        return (
          <div className="space-y-3">
            <div>
              <label className="text-[10px] font-medium text-gray-500 uppercase tracking-wider">Synth Family</label>
              <div className="grid grid-cols-2 gap-1.5 mt-1">
                {["subtractive", "fm", "wavetable", "granular"].map(f => (
                  <button key={f} onClick={() => setSynthFamily(f)}
            ...
            <SliderControl label="Brightness" value={Math.round(macroBrightness * 100)} onChange={v => setMacroBrightness(v / 100)} />
            <SliderControl label="Movement" value={Math.round(macroMovement * 100)} onChange={v => setMacroMovement(v / 100)} />
            <SliderControl label="Bite" value={Math.round(macroBite * 100)} onChange={v => setMacroBite(v / 100)} />
            <SliderControl label="Space" value={Math.round(macroSpace * 100)} onChange={v => setMacroSpace(v / 100)} />
          </div>
        );
      case "ensemble":
        return (
          <div className="space-y-3">
            ...
            <div>
              <CheckboxOption label="Enable Vocalist" checked={vocalistEnabled} onChange={setVocalistEnabled} />
              {vocalistEnabled && (
                <div className="flex items-center gap-1.5 mt-1.5">
                  <AlertTriangle className="w-3 h-3 text-amber-500" />
                  <span className="text-[10px] text-amber-600">Vocalist audio rendering is BETA quality</span>
                </div>
              )}
            </div>
          </div>
  ...
  if (crateState !== "open") {
    return (
      <div className="h-[100dvh] bg-white flex flex-col items-center justify-center overflow-hidden" style={{ colorScheme: "light" }}>
        <div
          ...
          {crateState === "opening" && (
            <div className="absolute inset-0 flex items-end justify-center pb-4">
              <div className="px-4 py-1.5 bg-white/10 backdrop-blur-md rounded-full">
                <span className="text-gray-500 text-xs font-medium tracking-wider">Loading instrument...</span>
              </div>
            </div>
          )}
        </div>
        {crateState === "closed" && (
          <p className="mt-8 text-gray-500 text-xs tracking-widest uppercase animate-pulse">Tap to open</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col text-gray-900 bg-white" style={{ colorScheme: "light" }}>
      <style>{`
        .holo-gradient {
          position: relative;
          overflow: hidden;
      ...
      <input ref={fileInputRef} type="file" accept="audio/mpeg,audio/wav,audio/x-wav,audio/ogg,audio/aac,audio/m4a,.mp3,.wav,.ogg,.m4a,.aac" className="hidden" onChange={handleFileChange} data-testid="input-audio-file" />
      ...
      <nav className="h-11 sm:h-12 border-b border-gray-200 flex-shrink-0 z-50 bg-white">
        <div className="max-w-[1400px] mx-auto px-3 sm:px-4 h-full flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <Link href="/"><Image src={svivvaLogo} alt="Svivva" width={100} height={32} className="h-5 w-auto object-contain" /></Link>
            <span className="text-gray-300 hidden sm:inline">/</span>
            <span className="font-semibold text-gray-900 text-xs sm:text-sm">Play</span>
            <Badge className="bg-[#A05068]/10 text-[#B87888] text-[8px] sm:text-[9px] border-[#A05068]/30 hidden sm:flex">MIDI: Professional</Badge>
            <Badge className="bg-amber-100 text-amber-700 text-[8px] sm:text-[9px] border-amber-200 hidden md:flex">Audio: Beta</Badge>
          </div>
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <Button onClick={() => setShowGuidedBuilder(true)} variant="outline" size="sm" className="gap-1 border-gray-300 text-gray-600 text-[10px] sm:text-xs hidden sm:flex hover:bg-gray-50" data-testid="button-guided-builder">
              <Wand2 className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
              Guided Builder
            </Button>
            <Link href="/"><Button variant="outline" size="sm" className="gap-1 border-gray-300 text-gray-600 text-[10px] sm:text-xs hover:bg-gray-50" data-testid="button-back-home">Back</Button></Link>
          </div>
        </div>
      </nav>
          ...
          <div className="rounded-xl flex flex-col overflow-hidden"
            ...
            <div className="px-3 sm:px-5 pt-3 sm:pt-4 pb-2 sm:pb-3 flex-shrink-0">
              <div className="flex items-center gap-3">
                <div className="flex-1 rounded-md px-3 py-1.5 sm:py-2 flex flex-col gap-1 min-w-0"
                  style={{
                  ...
                  {audioName ? (
                    <>
                      ...
                      <div className="flex items-center gap-[1px] w-full h-6" data-testid="audio-waveform">
                        {Array.from({ length: 150 }).map((_, i) => {
                          const h = Math.abs(Math.sin(i * 0.25 + 0.3) * 0.7 + Math.cos(i * 0.55 + 1.2) * 0.4 + Math.sin(i * 0.9 + 2) * 0.25);
                          return (
                            <div key={i} className="flex-1 rounded-[1px] bg-[#A05068]" style={{
                              height: `${Math.max(8, h * 100)}%`,
                              opacity: 0.35 + h * 0.55,
                              animation: isPlaying ? `waveformPulse 0.6s ease-in-out ${i * 0.015}s infinite alternate` : "none",
                            }} />
                          );
                        })}
                      </div>
                    ...
                    <span className="text-[10px] sm:text-xs text-gray-500 italic">No audio imported</span>
                  )}
                </div>
            ...
            <div className="flex flex-col lg:flex-row px-3 sm:px-5 pb-2 sm:pb-3 gap-3">
              <div className="flex-1 min-w-0 flex flex-col rounded-lg overflow-hidden"
                style={{
                  background: "linear-gradient(180deg, #111, #0a0a0a)",
                  boxShadow: "inset 3px 3px 10px rgba(0,0,0,0.7), inset -1px -1px 4px rgba(255,255,255,0.03)",
                  border: "2px solid #222",
                }}>
                <div className="border-b border-gray-700/50 px-2 sm:px-3 flex-shrink-0">
                  <div className="flex items-center gap-0.5 overflow-x-auto scrollbar-none">
                    {(Object.keys(MODE_CONFIG) as PlayMode[]).map(m => {
                      ...
                      return (
                        <button key={m} onClick={() => { if (!isComingSoon) setMode(m); }}
                          ...
                          <span className={isActive && !isComingSoon ? "holo-icon" : ""}><Icon className="w-3.5 h-3.5" /></span>
                          <span className={`sm:hidden ${isActive && !isComingSoon ? "holo-text" : ""}`}>{cfg.shortLabel}</span>
                          <span className={`hidden sm:inline ${isActive && !isComingSoon ? "holo-text" : ""}`}>{cfg.label}</span>
                          {isComingSoon && <Lock className="w-2.5 h-2.5 ml-0.5 text-gray-600" />}
                          {isActive && !isComingSoon && (
                            <span className="absolute bottom-0 left-2 right-2 h-[2px] rounded-full holo-gradient" style={{ background: "linear-gradient(90deg, #4A1E34 0%, #7A3850 10%, #6B2C5A 22%, #9A6878 34%, #5E3870 46%, #8A5868 56%, #4A8E90 68%, #7A4E78 78%, #4A1E34 88%, #7A3850 100%)" }}><HolographicNoise /></span>
                          )}
                        </button>
                      );
                    })}
                ...
                <div className="m-2 sm:m-3 rounded-md flex-1 flex flex-col"
                  style={{
                    background: "#0a0a0a",
                  ...
                  {!audioFile && mode !== "composition" && !isAnalyzing && stems.length === 0 && !patchResult && (
                    <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8 text-center overflow-y-auto">
                      ...
                      <p className="text-xs sm:text-sm text-gray-400 max-w-md mb-3 sm:mb-4">{MODE_CONFIG[mode].description}</p>
                      ...
                      <p className="text-[10px] text-gray-500 max-w-sm">Use the Import button above to analyze key, tempo, chords, and structure. Then generate professional MIDI output.</p>
                    </div>
                  )}
                  ...
                  {isAnalyzing && (
                    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                      ...
                      <p className="text-sm text-gray-400">Detecting key, tempo, chords, sections, and timbral characteristics...</p>
                    </div>
                  )}
                  ...
                  {!isAnalyzing && effectiveAnalysis && stems.length === 0 && !patchResult && !isGenerating && (
                    <div className="flex-1 flex flex-col p-3 sm:p-4 overflow-y-auto">
                      ...
                      <div className="mb-5 p-3 rounded-lg bg-[#0a1a1a] border border-gray-800">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="flex flex-col gap-1.5">
                            <label className="text-[10px] font-semibold text-gray-400 uppercase">Tempo (BPM)</label>
                            <input
                              type="number"
                              min="30"
                              max="300"
                      ...
                      {effectiveAnalysis.chords && effectiveAnalysis.chords.length > 0 && (
                        <div className="mb-5">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <button onClick={() => setShowChordEditor(!showChordEditor)} className="text-[10px] holo-text font-medium" data-testid="button-toggle-chord-editor">
                            ...
                            {effectiveAnalysis.chords.slice(0, 32).map((chord, i) => (
                              showChordEditor && editingChord === i ? (
                                <input key={i} autoFocus defaultValue={chord.symbol}
                                  ...
                                  onBlur={e => { handleChordEdit(i, e.target.value); setEditingChord(null); }}
                                  onKeyDown={e => { if (e.key === "Enter") { handleChordEdit(i, (e.target as HTMLInputElement).value); setEditingChord(null); } }}
                                  data-testid={`input-chord-edit-${i}`} />
                              ) : (
                                <button key={i} onClick={() => showChordEditor && setEditingChord(i)}
                                  className={`text-[10px] font-mono px-1.5 py-0.5 rounded border transition-colors ${
                                    chordEdits[i] ? "bg-[#A05068]/10 border-[#A05068]/30 holo-text" :
                                    showChordEditor ? "bg-gray-800 border-gray-700 text-gray-400 hover:border-[#A05068]" :
                                    "bg-gray-800 border-gray-700 text-gray-400"
                                  }`}
                                  data-testid={`button-chord-${i}`}>
                                  {chord.symbol}{chord.roman && !showChordEditor ? ` (${chord.roman})` : ""}
                                </button>
                              )
                            ))}
                      ...
                      {mode === "chords" && (
                        <div className="mb-4">
                          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Chord Browser</h3>
                          <div className="flex flex-wrap items-center gap-2 mb-3">
                            <select value={chordRoot} onChange={e => setChordRoot(e.target.value)} className="text-xs bg-[#1a1a1a] border border-gray-700 rounded px-2 py-1.5 text-gray-200 focus:outline-none focus:border-[#A05068]" data-testid="select-chord-root">
                              {["C","C#","D","Eb","E","F","F#","G","Ab","A","Bb","B"].map(r => <option key={r} value={r}>{r}</option>)}
                            </select>
                            <select value={chordTagFilter} onChange={e => setChordTagFilter(e.target.value)} className="text-xs bg-[#1a1a1a] border border-gray-700 rounded px-2 py-1.5 text-gray-200 focus:outline-none focus:border-[#A05068]" data-testid="select-chord-tag">
                              <option value="">All Styles</option>
                              {["neo-soul","jazz","brazilian","gospel","quartal","steely-dan","minimal","altered"].map(t => <option key={t} value={t}>{t}</option>)}
                            </select>
                              ...
                              {ChordKit.chordsInKey(chordRoot, /minor|m$/i.test(manualKey ?? effectiveAnalysis.key) ? "minor" : "major").map(dc => {
                                ...
                                return (
                                  <button key={dc.degree} onClick={() => { setSelectedChordId(dc.chordId); setChordRoot(ChordKit.rootLabel(dc.rootPc)); }}
                                    ...
                                    <span className="text-[10px] font-semibold text-gray-300">{dc.roman}</span>
                                    <span className="text-[9px] text-gray-500 mt-0.5">{dc.symbol}</span>
                                    {v && <span className="text-[8px] text-gray-600 mt-0.5 font-mono">{v.notes.join(" ")}</span>}
                                  </button>
                                );
                              })}
                            </div>
                          </div>
                          <div className="max-h-48 overflow-y-auto grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-1.5" data-testid="chord-browser-grid">
                            {ChordKit.list({ tag: chordTagFilter || undefined, q: chordSearch || undefined }).slice(0, 48).map(c => {
                              const sym = ChordKit.displaySymbol(chordRoot, c.id);
                              const isSelected = selectedChordId === c.id;
                              return (
                                <button key={c.id} onClick={() => { setSelectedChordId(c.id); setChordInversion(0); }}
                                  ...
                                  <div className="text-[9px] text-gray-500 truncate">{c.name}</div>
                                  <div className="flex flex-wrap gap-0.5 mt-1">{c.tags.slice(0, 3).map(t => <span key={t} className="text-[7px] px-1 py-0.5 bg-gray-800 rounded text-gray-500">{t}</span>)}</div>
                                </button>
                              );
                            })}
                          </div>
                          {selectedChordId && (() => {
                            const v = ChordKit.voicing(chordRoot, selectedChordId, { inversion: chordInversion, octave: 3 });
                            if (!v) return null;
                            return (
                              <div className="mt-3 p-3 rounded-lg bg-[#0d1a1a] border border-[#A05068]/30">
                                <div className="flex items-center justify-between mb-2">
                                  <span className="text-sm font-semibold text-gray-200">{ChordKit.displaySymbol(chordRoot, selectedChordId)}</span>
                                  <div className="flex items-center gap-2">
                                    <label className="text-[9px] text-gray-400">Inversion</label>
                                    <select value={chordInversion} onChange={e => setChordInversion(Number(e.target.value))} className="text-[10px] bg-[#1a1a1a] border border-gray-700 rounded px-1.5 py-0.5 text-gray-200" data-testid="select-chord-inversion">
                                      {[0,1,2,3].map(i => <option key={i} value={i}>{i === 0 ? "Root" : `${i}${i===1?"st":i===2?"nd":"rd"}`}</option>)}
                                    </select>
                                  </div>
                                </div>
                                <div className="flex flex-wrap gap-1 mb-1">{v.notes.map((n, i) => <span key={i} className="text-xs font-mono px-1.5 py-0.5 bg-gray-800 rounded text-[#B87888]">{n}</span>)}</div>
                                <div className="text-[9px] text-gray-500 font-mono">MIDI: {v.midi.join(", ")}</div>
                              </div>
                            );
                          })()}
                        </div>
                      ...
                      {mode === "composition" && (
                        <div className="mb-4">
                          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Counterpoint & Hocketing</h3>
                          <div className="grid grid-cols-2 gap-3 mb-3">
                            <button onClick={() => setReichType("counterpoint")}
                              ...
                              <div className="text-xs font-medium text-gray-200">Counterpoint</div>
                              <div className="text-[10px] text-gray-400 mt-0.5">3-voice interlocking canons</div>
                            </button>
                            <button onClick={() => setReichType("hocket")}
                              ...
                              <div className="text-[10px] text-gray-400 mt-0.5">6-voice alternating texture</div>
                            </button>
                          </div>
                          <div className="grid grid-cols-2 gap-3 mb-3">
                            <div className="flex flex-col gap-1.5">
                              <label className="text-[10px] font-semibold text-gray-400 uppercase">Style</label>
                              <select value={reichStyle} onChange={e => setReichStyle(e.target.value as ReichStyle)} className="text-xs bg-[#1a1a1a] border border-gray-700 rounded px-2 py-1.5 text-gray-200 focus:outline-none focus:border-[#A05068]" data-testid="select-reich-style">
                            ...
                            <div className="flex flex-col gap-1.5">
                              <label className="text-[10px] font-semibold text-gray-400 uppercase">Scale</label>
                              <select value={reichScale} onChange={e => setReichScale(e.target.value)} className="text-xs bg-[#1a1a1a] border border-gray-700 rounded px-2 py-1.5 text-gray-200 focus:outline-none focus:border-[#A05068]" data-testid="select-reich-scale">
                                {listScales().map(s => <option key={s} value={s}>{s.replace(/_/g, " ")}</option>)}
                              </select>
                            </div>
                          </div>
                      ...
                      {mode !== "chords" && (
                        <>
                          <div className="mb-4">
                            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Style Preset</h3>
                      ...
                      <div className="mb-4 p-3 bg-gray-800/30 rounded-lg border border-gray-700">
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Seed Control</span>
                          <label className="flex items-center gap-1.5 text-[10px] text-gray-400 cursor-pointer">
                            <input type="checkbox" checked={useSeed} onChange={e => setUseSeed(e.target.checked)} className="accent-[#A05068]" data-testid="checkbox-use-seed" />
                            Lock seed
                          </label>
                        </div>
                        <div className="flex items-center gap-2">
                          <input type="number" value={seed} onChange={e => setSeed(Number(e.target.value))}
                            ...
                            disabled={!useSeed} data-testid="input-seed" />
                          <button onClick={() => setSeed(Math.floor(Math.random() * 999999))} className="text-[10px] holo-text font-medium" disabled={!useSeed} data-testid="button-randomize-seed">Randomize</button>
                        </div>
                        <p className="text-[9px] text-gray-400 mt-1">{useSeed ? "Same seed = reproducible output" : "Random seed each generation"}</p>
                      </div>

                      <div className="flex flex-wrap items-center gap-2 mt-auto pt-2 sm:pt-3">
                        {mode === "composition" ? (
                          <Button onClick={handleLocalCompositionGenerate} className="gap-1.5 sm:gap-2 text-white text-xs sm:text-sm holo-gradient" style={{ background: "linear-gradient(135deg, #4A1E34 0%, #7A3850 10%, #6B2C5A 22%, #9A6878 34%, #5E3870 46%, #8A5868 56%, #4A8E90 68%, #7A4E78 78%, #4A1E34 88%, #7A3850 100%)" }} disabled={isGenerating} data-testid="button-generate-composition">
                            <HolographicNoise /><Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Generate {reichType === "counterpoint" ? "Counterpoint" : "Hocket"}
                          </Button>
                        ) : mode === "chords" ? (
                          <div className="text-[10px] text-gray-500">Select chords above to explore voicings</div>
                        ) : (
                          <>
                            <Button onClick={() => handleGenerate("preview")} className="gap-1.5 sm:gap-2 text-white text-xs sm:text-sm holo-gradient" style={{ background: "linear-gradient(135deg, #4A1E34 0%, #7A3850 10%, #6B2C5A 22%, #9A6878 34%, #5E3870 46%, #8A5868 56%, #4A8E90 68%, #7A4E78 78%, #4A1E34 88%, #7A3850 100%)" }} disabled={isGenerating} data-testid="button-generate-preview">
                              <HolographicNoise /><Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Preview (8-16 bars)
                            </Button>
                            <Button onClick={() => handleGenerate("full")} variant="outline" className="gap-1.5 sm:gap-2 border-gray-700 text-xs sm:text-sm" disabled={isGenerating} data-testid="button-generate-full">
                              <Zap className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Full Render
                            </Button>
                          </>
                        )}
                        <Button onClick={() => setShowSettings(!showSettings)} variant="outline" size="icon" className="border-gray-700 ml-auto" data-testid="button-toggle-settings">
                      ...
                      {showSettings && (
                        <div className="mt-3 p-4 bg-gray-800/30 rounded-lg border border-gray-700">
                          <h4 className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-3">{MODE_CONFIG[mode].label} Settings</h4>
                          {renderModeSettings()}
                        </div>
                      )}
                    </div>
                  ...
                  {isGenerating && (
                    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                      ...
                      <div className="w-64 space-y-1 mb-4">
                        <PipelineStep label="Analysis" done active={pipelineStage.includes("Analyzing")} />
                        <PipelineStep label="Arrangement Planning" done={pipelineStage.includes("MIDI") || pipelineStage.includes("Complete") || pipelineStage.includes("take")} active={pipelineStage.includes("Planning")} />
                        <PipelineStep label={mode === "patch" ? "Patch Design" : mode === "solo" ? "Solo Takes (3x)" : "MIDI Generation"} done={pipelineStage === "Complete"} active={pipelineStage.includes("MIDI") || pipelineStage.includes("Designing") || pipelineStage.includes("take") || pipelineStage.includes("solo")} />
                      </div>
                      <Button onClick={() => { setIsGenerating(false); setPipelineStage("Cancelled"); }} variant="outline" size="sm" className="text-xs gap-1 border-gray-700 text-gray-400" data-testid="button-cancel-generation">
                    ...
                    <div className="flex-1 flex flex-col p-2 sm:p-4 overflow-y-auto">
                      <div className="flex items-center justify-between gap-2 sm:gap-3 mb-2 flex-wrap">
                        <div className="min-w-0">
                          <h3 className="text-xs sm:text-sm font-semibold text-gray-200">Generated Stems ({stems.length})</h3>
                          {planInfo?.chordProgression && planInfo.chordProgression.length > 0 ? (
                            ...
                            <p className="text-[9px] sm:text-[10px] text-gray-400 mt-0.5 truncate">{planInfo.form?.total_bars || "16"} bars | seed: {seed}</p>
                          )}
                        </div>
                        <div className="flex items-center gap-1 sm:gap-1.5 flex-wrap">
                          <Badge className="bg-[#A05068]/20 text-[#B87888] text-[8px] sm:text-[9px] border-[#A05068]/30 hidden sm:flex">MIDI: Professional</Badge>
                          <Button onClick={() => handleGenerate("preview")} variant="outline" size="sm" className="text-[10px] sm:text-xs gap-1 border-gray-700" data-testid="button-regenerate">
                            <RotateCcw className="w-3 h-3" /> <span className="hidden sm:inline">Regenerate All</span><span className="sm:hidden">Regen</span>
                          </Button>
                        </div>
                      </div>
                      ...
                      {versionHistory.length > 1 && (
                        <div className="mb-3 p-2 bg-gray-800/30 rounded-lg border border-gray-700">
                          <div className="flex items-center gap-2 mb-1.5">
                            ...
                            <Badge variant="secondary" className="text-[9px]">{versionHistory.length} versions</Badge>
                          </div>
                          <div className="flex gap-1 overflow-x-auto">
                            {versionHistory.map((ver, vi) => (
                              <button key={ver.id} onClick={() => handleVersionSwitch(vi)}
                                className={`flex-shrink-0 px-2 py-1 rounded text-[10px] font-mono border transition-colors ${
                                  vi === activeVersion ? "bg-[#A05068]/20 border-[#A05068] holo-text" : "bg-gray-800 border-gray-700 text-gray-400 hover:border-gray-500"
                                }`}
                      ...
                      {mode === "solo" && soloTakes.length > 1 && (
                        <div className="mb-3 p-2 bg-[#A05068]/10 rounded-lg border border-[#A05068]/30">
                          <span className="text-[10px] font-semibold holo-text uppercase tracking-wider block mb-1.5">Solo Takes</span>
                          <div className="flex gap-1.5">
                            {soloTakes.map((take, ti) => (
                              <button key={ti} onClick={() => { setActiveTake(ti); setStems(take.stems); }}
                                className={`flex-1 px-2 py-1.5 rounded text-xs font-medium border transition-colors ${
                                  ti === activeTake ? "bg-[#A05068]/20 border-[#A05068] holo-text" : "bg-gray-800 border-gray-700 text-gray-400"
                                }`}
                                data-testid={`button-take-${ti}`}>
                                Take {ti + 1}
                                <span className="block text-[9px] font-mono text-gray-400">seed: {take.seed}</span>
                              </button>
                            ))}
                          </div>
                          <p className="text-[9px] text-gray-400 mt-1.5">Compare takes and pick the best solo variation</p>
                        </div>
                      )}
                        ...
                        {stems.map((stem, i) => (
                          <div key={stem.id || i}
                            className={`flex items-center gap-1.5 sm:gap-3 p-1.5 sm:p-2.5 rounded-lg border transition-colors ${
                              stem.muted ? "bg-gray-800/30 border-gray-700 opacity-50" :
                              lockedStems.has(stem.name) ? "bg-[#A05068]/10 border-[#A05068]/30" :
                              "bg-gray-800/20 border-gray-700"
                            }`}
                            data-testid={`stem-${i}`}>
                            <div className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full flex-shrink-0 ${lockedStems.has(stem.name) ? "bg-[#A05068]" : "bg-[#A05068]"}`} />
                            <div className="min-w-0 flex-1">
                              <div className="text-[10px] sm:text-xs font-medium text-gray-200 truncate">{stem.name}</div>
                              <div className="text-[9px] sm:text-[10px] text-gray-400 truncate">{stem.instrumentHint} / {stem.role}</div>
                            </div>
                            {stem.articulations && stem.articulations.length > 0 && (
                              <div className="hidden md:flex items-center gap-1">
                                {stem.articulations.slice(0, 2).map((a, j) => (
                                  <Badge key={j} variant="secondary" className="text-[8px]">{a}</Badge>
                                ))}
                              </div>
                            )}
                            <div className="flex items-center gap-0">
                              <button onClick={() => toggleStemLock(stem.name)}
                                className={`p-0.5 sm:p-1 rounded ${lockedStems.has(stem.name) ? "" : "text-gray-600"}`}
                                title={lockedStems.has(stem.name) ? "Unlock stem" : "Lock stem"} data-testid={`button-lock-stem-${i}`}>
                                {lockedStems.has(stem.name) ? <span className="holo-icon"><Lock className="w-2.5 h-2.5 sm:w-3 sm:h-3" /></span> : <Unlock className="w-2.5 h-2.5 sm:w-3 sm:h-3" />}
                              </button>
                              <button onClick={() => handleRegenerateStem(stem.name)}
                                disabled={lockedStems.has(stem.name) || regeneratingStem === stem.name}
                                className={`p-0.5 sm:p-1 rounded ${lockedStems.has(stem.name) ? "text-gray-200" : "text-gray-400 hover:text-[#B87888]"}`}
                                title="Regenerate this stem" data-testid={`button-regen-stem-${i}`}>
                                {regeneratingStem === stem.name ? <Loader2 className="w-2.5 h-2.5 sm:w-3 sm:h-3 animate-spin" /> : <RotateCcw className="w-2.5 h-2.5 sm:w-3 sm:h-3" />}
                              </button>
                              <button onClick={() => { const u = [...stems]; u[i] = { ...u[i], muted: !u[i].muted }; setStems(u); }}
                                className={`p-0.5 sm:p-1 rounded ${stem.muted ? "text-red-400" : "text-gray-400"}`} title={stem.muted ? "Unmute" : "Mute"}>
                                {stem.muted ? <VolumeX className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> : <Volume2 className="w-3 h-3 sm:w-3.5 sm:h-3.5" />}
                              </button>
                              <button onClick={() => { const u = [...stems]; u[i] = { ...u[i], soloed: !u[i].soloed }; setStems(u); }}
                                className={`p-0.5 sm:p-1 rounded text-[9px] sm:text-[10px] font-bold ${stem.soloed ? "holo-text" : "text-gray-400"}`} title="Solo">S</button>
                            </div>
                            <div className="w-14 hidden sm:block">
                              <input type="range" min="-100" max="100" value={stem.pan}
                                onChange={e => {
                                  const pan = Number(e.target.value);
                                  const u = [...stems]; u[i] = { ...u[i], pan }; setStems(u);
                                  if (engineReady) getSoundEngine().updateMix(stem.name, { pan });
                                }}
                                className="w-full accent-[#A05068]" title={`Pan: ${stem.pan}`} />
                            </div>
                          </div>
                        ))}
                      </div>
                      {planInfo?.meendApplicableStems && planInfo.meendApplicableStems.length > 0 && meend && (
                        <div className="mt-3 p-2 bg-[#A05068]/10 rounded border border-[#A05068]/30">
                          <p className="text-[10px] holo-text">Meend (continuous pitch bend) applied to: {planInfo.meendApplicableStems.join(", ")}</p>
                        </div>
                      )}
                  ...
                  {patchResult && !isGenerating && (
                    <div className="flex-1 flex flex-col p-3 sm:p-4 overflow-y-auto">
                      <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
                        <div>
                          <h3 className="text-sm font-semibold text-gray-200">{patchResult.name}</h3>
                          <p className="text-[10px] text-gray-400 mt-0.5">{patchResult.synth_family} | {patchResult.mono_poly || "poly"}</p>
                        </div>
                        <Badge className="bg-[#A05068]/20 holo-text text-[9px] border-[#A05068]/30">Professional</Badge>
                      </div>
                        ...
                        {Object.entries(patchResult.macros).map(([key, val]) => (
                          <div key={key} className="bg-gray-800/50 rounded-lg p-3 text-center">
                            <div className="w-10 h-10 mx-auto rounded-full border-[2px] border-gray-700 flex items-center justify-center relative mb-1.5">
                              <div className="absolute w-0.5 h-4 bg-[#A05068] rounded-full origin-bottom"
                                style={{ transform: `rotate(${(val as number) * 270 - 135}deg)`, bottom: "50%" }} />
                            </div>
                            <div className="text-[10px] font-medium text-gray-200 capitalize">{key}</div>
                            <div className="text-[9px] text-gray-500">{Math.round((val as number) * 100)}%</div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                        <div>
                          <h4 className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Oscillators</h4>
                          <div className="space-y-1">
                            {patchResult.oscillators.map((osc, i) => (
                              <div key={i} className="bg-gray-800/50 rounded px-2 py-1.5 text-[10px] font-mono flex items-center gap-2">
                                <span className="holo-text font-semibold">{osc.shape}</span>
                                <span className="text-gray-400">Oct {osc.oct > 0 ? `+${osc.oct}` : osc.oct}</span>
                                <span className="text-gray-400">Mix {Math.round(osc.mix * 100)}%</span>
                              </div>
                            ))}
                          </div>
                        </div>
                        <div>
                          <h4 className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Filter</h4>
                          <div className="bg-gray-800/50 rounded px-2 py-1.5 text-[10px] font-mono space-y-0.5">
                            <div>{patchResult.filter.type} | {patchResult.filter.cutoff_hz}Hz</div>
                            <div>Res: {Math.round(patchResult.filter.resonance * 100)}% | Drive: {Math.round((patchResult.filter.drive || 0) * 100)}%</div>
                          </div>
                          <h4 className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5 mt-3">Envelope</h4>
                          <div className="bg-gray-800/50 rounded px-2 py-1.5 text-[10px] font-mono space-y-0.5">
                            <div>Amp: A{patchResult.env.amp.A} D{patchResult.env.amp.D} S{patchResult.env.amp.S} R{patchResult.env.amp.R}</div>
                            <div>Filt: A{patchResult.env.filter.A} D{patchResult.env.filter.D} S{patchResult.env.filter.S} R{patchResult.env.filter.R}</div>
                          </div>
                        </div>
                      </div>
                      ...
                      {patchResult.instructions && (
                        <div className="mb-4">
                          <h4 className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Playing Instructions</h4>
                          <div className="bg-gray-800/50 rounded p-3 text-[11px] text-gray-400 whitespace-pre-wrap leading-relaxed">{patchResult.instructions}</div>
                        </div>
                      )}

                      {patchResult.mappings?.midi_cc && typeof patchResult.mappings.midi_cc === "object" && (
                        <MidiCcMappings data={patchResult.mappings.midi_cc as Record<string, number>} />
                      )}

                      <div className="flex items-center gap-2 mt-auto pt-3">
                        <Button onClick={() => handleGenerate("preview")} variant="outline" size="sm" className="text-xs gap-1 border-gray-700" data-testid="button-regenerate-patch">
              ...
              <div className="hidden lg:flex w-48 xl:w-56 p-3 xl:p-4 flex-col gap-3 flex-shrink-0 overflow-y-auto">
                ...
                <div className="hidden lg:block mt-4 pt-3 border-t border-gray-700/50">
                  <h4 className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Quality Tiers</h4>
                  <div className="space-y-1.5 text-[10px]">
                    <div className="flex items-start gap-1.5">
                      <div className="w-1.5 h-1.5 rounded-full bg-[#A05068] mt-1 flex-shrink-0" />
                      <span className="text-gray-400"><span className="font-medium">Professional:</span> MIDI, analysis, patches</span>
                    </div>
                    <div className="flex items-start gap-1.5">
                      <div className="w-1.5 h-1.5 rounded-full bg-amber-500 mt-1 flex-shrink-0" />
                      <span className="text-gray-400"><span className="font-medium">Beta:</span> Synth audio & WAV export</span>
                    </div>
                    <div className="flex items-start gap-1.5">
                      <div className="w-1.5 h-1.5 rounded-full bg-gray-500 mt-1 flex-shrink-0" />
                      <span className="text-gray-400"><span className="font-medium">Unavailable:</span> Vocal Swap</span>
                    </div>
                  </div>
                  <div className="mt-3 p-2 rounded border bg-gray-800/30 border-gray-700/30">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <Badge variant="secondary" className="text-[8px] bg-gray-700 text-gray-400">ROADMAP</Badge>
                    </div>
                    <p className="text-[9px] text-gray-500">Real-time vocal timbre conversion. Coming soon.</p>
                  </div>

                  <div className="mt-3 pt-3 border-t border-gray-700/50">
                    <button onClick={() => setShowNeuralPanel(!showNeuralPanel)}
                      className="flex items-center gap-1.5 w-full text-left"
                      data-testid="button-toggle-neural">
                      <span className="holo-icon"><BrainCircuit className="w-3.5 h-3.5" /></span>
                      <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Neural Audio</span>
                      <Badge className="text-[7px] text-white ml-auto holo-gradient" style={{ background: "linear-gradient(135deg, #4A1E34 0%, #7A3850 10%, #6B2C5A 22%, #9A6878 34%, #5E3870 46%, #8A5868 56%, #4A8E90 68%, #7A4E78 78%, #4A1E34 88%, #7A3850 100%)" }}><HolographicNoise />AI</Badge>
                    </button>
                    {showNeuralPanel && (
                      <div className="mt-2 space-y-2">
                        <div>
                          <label className="text-[9px] text-gray-400">Genre</label>
                          <input value={neuralGenre} onChange={e => setNeuralGenre(e.target.value)}
                        ...
                        <div>
                          ...
                          <input value={neuralMood} onChange={e => setNeuralMood(e.target.value)}
                        ...
                        <div>
                          ...
                          <select value={neuralEnergy} onChange={e => setNeuralEnergy(e.target.value)}
                            className="w-full border border-gray-600 rounded px-2 py-1 text-[10px] focus:outline-none focus:border-[#A05068] mt-0.5 bg-gray-800/50 text-gray-200"
                        ...
                        <Button onClick={handleGenerateNeuralPrompt}
                          disabled={isGeneratingPrompt || stems.length === 0}
                          className="w-full text-[10px] text-white gap-1 holo-gradient" style={{ background: "linear-gradient(135deg, #4A1E34 0%, #7A3850 10%, #6B2C5A 22%, #9A6878 34%, #5E3870 46%, #8A5868 56%, #4A8E90 68%, #7A4E78 78%, #4A1E34 88%, #7A3850 100%)" }}
                          size="sm"
                          data-testid="button-generate-neural-prompt">
                          <HolographicNoise />{isGeneratingPrompt ? <Loader2 className="w-3 h-3 animate-spin" /> : <BrainCircuit className="w-3 h-3" />}
                          {isGeneratingPrompt ? "Generating..." : "Generate Neural Prompt"}
                        </Button>
                        {stems.length === 0 && (
                          <p className="text-[9px] text-gray-500">Generate stems first to create a neural audio prompt.</p>
                        )}
                        {neuralPromptResult && (
                          <div className="space-y-1.5 mt-2">
                            <div className="flex items-center justify-between">
                              <span className="text-[9px] font-medium text-gray-400">Generated Prompt</span>
                              <div className="flex items-center gap-1">
                                <Badge className="text-[7px] text-white holo-gradient" style={{ background: "linear-gradient(135deg, #4A1E34 0%, #7A3850 10%, #6B2C5A 22%, #9A6878 34%, #5E3870 46%, #8A5868 56%, #4A8E90 68%, #7A4E78 78%, #4A1E34 88%, #7A3850 100%)" }}><HolographicNoise />Score: {neuralPromptResult.qualityScore}</Badge>
                                <button onClick={copyNeuralPrompt} className="p-0.5 text-gray-400 hover:text-[#B87888]" data-testid="button-copy-neural-prompt">
                                  {promptCopied ? <span className="holo-icon"><Check className="w-3 h-3" /></span> : <Copy className="w-3 h-3" />}
                                </button>
                              </div>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-0.5">
                              {neuralPromptResult.tags.slice(0, 6).map((tag, i) => (
                                <Badge key={i} variant="secondary" className="text-[7px]">{tag}</Badge>
                              ))}
                            </div>
                            <div className="text-[8px] text-gray-500 space-y-0.5">
                              <div>Steps: {neuralPromptResult.modelSettings.steps} | CFG: {neuralPromptResult.modelSettings.cfgScale}</div>
                              <div>Duration: {neuralPromptResult.modelSettings.duration}s</div>
                            </div>
                          </div>
                        )}
                        <a href="/dashboard/neural-audio" target="_blank" rel="noopener noreferrer"
            ...
            <div className="px-3 sm:px-5 pt-1 relative">
              <button onClick={() => setShowExportMenu(!showExportMenu)}
                ...
                style={{
                  background: (stems.length > 0 || patchResult) ? "linear-gradient(180deg, #5BA8A0, #4A8890)" : "linear-gradient(180deg, #444, #333)",
                  boxShadow: "2px 3px 6px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.1)",
              ...
              {showExportMenu && (
                <div className="absolute bottom-full left-3 right-3 mb-2 bg-[#1a1a1a] border border-gray-700 rounded-lg shadow-lg z-50" data-testid="export-menu">
                  <button onClick={() => { handleExport("midi"); setShowExportMenu(false); }}
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-gray-400">For your DAW (Ableton, Logic, etc)</div>
                    </div>
                  </button>
                  <button onClick={() => { handleExport("json"); setShowExportMenu(false); }}
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-gray-400">Complete session data & metadata</div>
                    </div>
                  </button>
                </div>
              )}
            ...
            <div className="px-3 sm:px-5 pb-3 sm:pb-4 pt-1 flex items-center gap-2 sm:gap-3 flex-shrink-0">
              <div className="flex items-center gap-1">
                {[
                  { onClick: () => {
                    if (engineReady && stems.length > 0) { const engine = getSoundEngine(); engine.seek(Math.max(0, engine.getPosition() - 5)); setPlaybackPos(engine.getPosition()); }
                    else if (audioRef.current) { audioRef.current.currentTime = Math.max(0, audioRef.current.currentTime - 5); }
                  }, disabled: !audioUrl && !engineReady, icon: <SkipBack className="w-4 h-4" />, testId: "button-skip-back" },
                  { onClick: togglePlay, disabled: !audioUrl && !engineReady, icon: isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />, testId: "button-play-pause" },
                  { onClick: stopPlayback, disabled: !engineReady, icon: <Square className="w-3.5 h-3.5" />, testId: "button-stop" },
                  { onClick: () => {
                    if (engineReady && stems.length > 0) { const engine = getSoundEngine(); engine.seek(Math.min(playbackDuration, engine.getPosition() + 5)); setPlaybackPos(engine.getPosition()); }
                    else if (audioRef.current) { audioRef.current.currentTime = Math.min(audioRef.current.duration, audioRef.current.currentTime + 5); }
                  }, disabled: !audioUrl && !engineReady, icon: <SkipForward className="w-4 h-4" />, testId: "button-skip-forward" },
                ].map((btn) => (
                  <button key={btn.testId} onClick={btn.onClick} disabled={btn.disabled}
              ...
              <div className="flex items-center gap-1 sm:gap-1.5 text-[9px] sm:text-[10px] text-gray-300 font-mono flex-shrink-0 font-bold">
                {engineReady ? `${formatTime(playbackPos)}/${formatTime(playbackDuration)}` : "--:--"}
              </div>
              <div className="flex-1 h-2 rounded-full overflow-hidden cursor-pointer" style={{ background: "linear-gradient(180deg, #333, #444)", boxShadow: "inset 1px 1px 3px rgba(0,0,0,0.5)" }} onClick={(e) => {
                ...
                <div className="h-full rounded-full transition-all holo-gradient" style={{ background: "linear-gradient(90deg, #4A1E34 0%, #7A3850 10%, #6B2C5A 22%, #9A6878 34%, #5E3870 46%, #8A5868 56%, #4A8E90 68%, #7A4E78 78%, #4A1E34 88%, #7A3850 100%)", width: playbackDuration > 0 ? `${(playbackPos / playbackDuration) * 100}%` : "0%" }}><HolographicNoise /></div>
              </div>
              <div className="hidden sm:flex items-center gap-2">
                <Volume2 className="w-3.5 h-3.5 text-gray-500" />
                <input type="range" min="0" max="100" value={masterVolume} className="w-14 accent-[#A05068]" onChange={e => {
              ...
              {pipelineStage && <Badge variant="secondary" className="text-[10px] hidden sm:flex">{pipelineStage}</Badge>}
            </div>
          </div>
      ...
      {showGuidedBuilder && <GuidedPromptBuilder onClose={() => setShowGuidedBuilder(false)} onApply={(prompt) => { setUserPrompt(prompt); setShowGuidedBuilder(false); }} currentMode={mode} />}
    </div>
  );
}
...
function SliderControl({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  return (
    <div>
      <label className="text-[10px] font-medium text-gray-500 flex items-center justify-between"><span>{label}</span><span className="text-gray-400">{value}%</span></label>
      <input type="range" min="0" max="100" value={value} onChange={e => onChange(Number(e.target.value))} className="w-full mt-0.5 accent-[#A05068]" />
    </div>
  );
}
...
function RadioOption({ label, checked, onChange }: { label: string; checked: boolean; onChange: () => void }) {
  return (
    <label className="flex items-center gap-1.5 text-[10px] font-medium text-gray-500 cursor-pointer">
      <input type="radio" checked={checked} onChange={onChange} className="accent-[#A05068]" />{label}
    </label>
  );
}
...
function CheckboxOption({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <label className="flex items-center gap-1.5 text-[10px] font-medium text-gray-500 cursor-pointer">
      <input type="checkbox" checked={checked} onChange={e => onChange(e.target.checked)} className="accent-[#A05068]" />{label}
    </label>
  );
}

function AnalysisCard({ label, value, sub, color }: { label: string; value: string; sub: string; color: string }) {
  return (
    <div className="bg-gray-800/50 rounded-lg p-2.5 text-center">
      <div className="text-xl font-bold" style={{ color }}>{value}</div>
      <div className="text-[9px] text-gray-400 mt-0.5">{label} ({sub})</div>
    </div>
  );
}
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between"><span className="text-gray-500">{label}</span><span className="text-gray-200 font-medium">{value}</span></div>
  );
}
...
function MidiCcMappings({ data }: { data: Record<string, number> }) {
  return (
    <div className="mb-4">
      <h4 className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5">MIDI CC Mappings</h4>
      <div className="bg-gray-800/50 rounded px-2 py-1.5 text-[10px] font-mono">
        {Object.entries(data).map(([key, val]) => (
          <span key={key} className="mr-3">{key}: CC{val}</span>
        ))}
      </div>
    </div>
  );
...
function PipelineStep({ label, done, active }: { label: string; done: boolean; active?: boolean }) {
  return (
    <div className={`flex items-center gap-2 px-2 py-1 rounded text-xs ${active ? "bg-[#A05068]/10 text-[#B87888]" : done ? "text-[#B87888]" : "text-gray-500"}`}>
      {active ? <Loader2 className="w-3 h-3 animate-spin" /> : done ? <div className="w-3 h-3 rounded-full flex items-center justify-center holo-gradient" style={{ background: "linear-gradient(135deg, #4A1E34 0%, #7A3850 10%, #6B2C5A 22%, #9A6878 34%, #5E3870 46%, #8A5868 56%, #4A8E90 68%, #7A4E78 78%, #4A1E34 88%, #7A3850 100%)" }}><HolographicNoise /><svg width="8" height="8" viewBox="0 0 8 8"><path d="M1 4l2 2 4-4" stroke="white" strokeWidth="1.5" fill="none" /></svg></div> : <div className="w-3 h-3 rounded-full border border-gray-700" />}
      <span className={active || done ? "holo-text" : ""}>{label}</span>
    </div>
  );
}

function GuidedPromptBuilder({ onClose, onApply, currentMode }: { onClose: () => void; onApply: (prompt: string) => void; currentMode: PlayMode }) {
  ...
  const INSTRUMENT_PALETTE = [
    ...
    { cat: "Strings", items: ["Violin", "Viola", "Cello", "Double Bass", "String Ensemble", "Harp"] },
    { cat: "Winds", items: ["Flute", "Oboe", "Clarinet", "Bassoon", "Trumpet", "Trombone", "French Horn", "Saxophone"] },
  ...
  const buildPrompt = () => {
    const allInstruments = [...selectedInstruments, ...(instruments ? instruments.split(",").map(s => s.trim()) : [])].filter(Boolean);
    const parts = [
      `Outcome: ${outcome}`,
      style && `Style: ${style}`,
    ...
    <div className="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} className="text-gray-500 hover:text-gray-500"><X className="w-4 h-4" /></button>
        </div>

        <div className="flex items-center gap-1 mb-4">
          {steps.map((s, i) => (
            <div key={i} className={`flex-1 h-1 rounded-full ${i <= step ? "holo-gradient" : "bg-gray-200"}`} style={i <= step ? { background: "linear-gradient(90deg, #4A1E34 0%, #7A3850 10%, #6B2C5A 22%, #9A6878 34%, #5E3870 46%, #8A5868 56%, #4A8E90 68%, #7A4E78 78%, #4A1E34 88%, #7A3850 100%)" } : undefined}><HolographicNoise /></div>
          ))}
        </div>
        <p className="text-xs text-gray-500 mb-4">Step {step + 1}/{steps.length}: {steps[step].desc}</p>

        <div className="min-h-[120px]">
          {step === 0 && (
            <div className="grid grid-cols-2 gap-2">
              {(["composition", "interpolation", "chords", "solo", "patch", "ensemble"] as PlayMode[]).map(m => (
                <button key={m} onClick={() => setOutcome(m)} className={`p-2 rounded-lg border text-left text-xs ${outcome === m ? "border-[#A05068] bg-[#A05068]/5" : "border-gray-200"}`}
          ...
          {step === 1 && (
            <textarea value={style} onChange={e => setStyle(e.target.value)} placeholder="e.g. 'airy minimalist with marimba textures' or 'neo-soul with extended jazz harmonies'"
              className="w-full border border-gray-200 rounded-lg p-3 text-sm resize-none h-24 focus:outline-none focus:border-[#A05068] bg-white text-gray-800" data-testid="input-guided-style" />
          )}
          {step === 2 && (
            <div className="space-y-3">
              {INSTRUMENT_PALETTE.map(cat => (
                <div key={cat.cat}>
                  <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">{cat.cat}</span>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {cat.items.map(inst => (
                      <button key={inst} onClick={() => toggleInstrument(inst)}
                        className={`text-[10px] px-2 py-1 rounded border transition-colors ${
                          selectedInstruments.includes(inst) ? "border-[#A05068] bg-[#A05068]/10 holo-text" : "border-gray-200 text-gray-500 hover:border-gray-400"
                        }`}
              ...
              {selectedInstruments.length > 0 && (
                <p className="text-[9px] holo-text">{selectedInstruments.length} selected: {selectedInstruments.join(", ")}</p>
              )}
            </div>
          )}
          {step === 3 && (
            <textarea value={instruments} onChange={e => setInstruments(e.target.value)} placeholder="Add custom instruments not in the palette, e.g. 'koto, tabla, kalimba'"
              className="w-full border border-gray-200 rounded-lg p-3 text-sm resize-none h-24 focus:outline-none focus:border-[#A05068] bg-white text-gray-800" data-testid="input-guided-instruments" />
          )}
          {step === 4 && (
            <div className="space-y-2">
              {["match", "reharmonize"].map(h => (
                <button key={h} onClick={() => setHarmonyPolicy(h)} className={`w-full p-3 rounded-lg border text-left text-xs ${harmonyPolicy === h ? "border-[#A05068] bg-[#A05068]/5" : "border-gray-200"}`}
                  data-testid={`button-guided-harmony-${h}`}>
                  <span className="font-medium capitalize">{h}</span>
                  <span className="text-gray-500 ml-2">{h === "match" ? "Keep original harmony" : "Allow creative reharmonization"}</span>
                </button>
              ))}
            </div>
          )}
          {step === 5 && (
            <div className="space-y-3">
              <div>
                ...
                <input value={scale} onChange={e => setScale(e.target.value)} placeholder="auto (or e.g. 'D dorian', 'Bb mixolydian')"
                  className="w-full border border-gray-200 rounded-lg p-2 text-sm focus:outline-none focus:border-[#A05068] mt-1 bg-white text-gray-800" data-testid="input-guided-scale" />
              </div>
              <label className="flex items-center gap-2 text-xs text-gray-500 cursor-pointer">
                <input type="checkbox" checked={meendEnabled} onChange={e => setMeendEnabled(e.target.checked)} className="accent-[#A05068]" data-testid="checkbox-guided-meend" />
          ...
          {step === 6 && (
            <div className="space-y-3">
              <div>
                ...
                <div className="grid grid-cols-3 gap-1.5 mt-1">
                  {[
                    ...
                    <button key={bc.id} onClick={() => setBuildCurve(bc.id)}
              ...
              <div>
                ...
                <input value={form} onChange={e => setForm(e.target.value)} placeholder="AABA, ABAB, through-composed..."
                  className="w-full border border-gray-200 rounded-lg p-2 text-sm focus:outline-none focus:border-[#A05068] mt-1 bg-white text-gray-800" data-testid="input-guided-form" />
              </div>
              <div>
                <label className="text-[10px] font-medium text-gray-500">Additional Notes</label>
                <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Any other instructions..."
                  className="w-full border border-gray-200 rounded-lg p-2 text-sm resize-none h-16 focus:outline-none focus:border-[#A05068] mt-1 bg-white text-gray-800" data-testid="input-guided-notes" />
              </div>
            </div>
          )}
          {step === 7 && (
            <div className="space-y-2">
              {([{ id: "preview", label: "Preview (8-16 bars)", desc: "Fast iteration, shorter output" }, { id: "full", label: "Full Render", desc: "Complete arrangement, all bars" }] as const).map(q => (
          ...
          {step === 8 && (
            <div className="bg-gray-50 rounded-lg p-3 text-xs text-gray-800 whitespace-pre-wrap leading-relaxed border border-gray-200">{buildPrompt()}</div>
          )}
        </div>

        <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-200">
          <Button onClick={() => setStep(Math.max(0, step - 1))} variant="outline" size="sm" disabled={step === 0} className="text-xs" data-testid="button-guided-back">Back</Button>
          {step < steps.length - 1 ? (
            <Button onClick={() => setStep(step + 1)} size="sm" className="text-xs text-white gap-1 holo-gradient" style={{ background: "linear-gradient(135deg, #4A1E34 0%, #7A3850 10%, #6B2C5A 22%, #9A6878 34%, #5E3870 46%, #8A5868 56%, #4A8E90 68%, #7A4E78 78%, #4A1E34 88%, #7A3850 100%)" }} data-testid="button-guided-next">
              <HolographicNoise />Next <ChevronRight className="w-3 h-3" />
            </Button>
          ) : (
            <Button onClick={() => onApply(buildPrompt())} size="sm" className="text-xs text-white gap-1 holo-gradient" style={{ background: "linear-gradient(135deg, #4A1E34 0%, #7A3850 10%, #6B2C5A 22%, #9A6878 34%, #5E3870 46%, #8A5868 56%, #4A8E90 68%, #7A4E78 78%, #4A1E34 88%, #7A3850 100%)" }} data-testid="button-guided-apply">
              <HolographicNoise /><Sparkles className="w-3 h-3" /> Apply Prompt
            </Button>
          )}
        </div>
      </div>

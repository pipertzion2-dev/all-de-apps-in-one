...
import { approveRoadmapItems } from "@/lib/orbit/roadmap";

export const dynamic = "force-dynamic";

type RouteParams = { params: Promise<{ id: string }> };

/** POST — approve proposed roadmap items (generates product specs). */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    const result = await approveRoadmapItems(projectId, user!.id, body);

...
export async function GET() {
  try {
    ...
    const [seoRows, blogRows, aeoRows, seedRows, credRows, miniRows, integRows, usecaseRows, templateRows, paaRows] = await Promise.all([
      db.select({ slug: seoLandingPages.slug })
      ...
      db.select({ indexnowKey: seedCredentials.indexnowKey, lastIndexnowSubmit: seedCredentials.lastIndexnowSubmit })
      ...
      db.select({ slug: seoLandingPages.slug })
        ...
        .where(eq(seoLandingPages.category, "seed-marketing"))
        .limit(50),
      db.select({ count: sql<number>`COUNT(*)` }).from(seoLandingPages).where(eq(seoLandingPages.category, "integration")),
      db.select({ count: sql<number>`COUNT(*)` }).from(seoLandingPages).where(eq(seoLandingPages.category, "usecase")),
      db.select({ count: sql<number>`COUNT(*)` }).from(seoLandingPages).where(eq(seoLandingPages.category, "template")),
      db.select({ count: sql<number>`COUNT(*)` }).from(seoLandingPages).where(eq(seoLandingPages.category, "paa")),
    ]);

    const comparisons = seoRows.filter((p) => p.slug.startsWith("svivva-vs-"));
    const hubPage = await db.select({ id: seoLandingPages.id })
      .from(seoLandingPages)
    ...
    const toolUrls = [
      ...seoRows.map((p) => ({ label: p.slug, url: `${BASE}/${p.slug}`, gscLink: gscBase + encodeURIComponent(`${BASE}/${p.slug}`) })),
      ...aeoRows.map((p) => ({ label: p.slug, url: `${BASE}/${p.slug}`, gscLink: gscBase + encodeURIComponent(`${BASE}/${p.slug}`) })),
      ...miniRows.map((p) => ({ label: p.slug, url: `${BASE}/${p.slug}`, gscLink: gscBase + encodeURIComponent(`${BASE}/${p.slug}`) })),
    ];

...
async function enhanceWithAI(
  examples: { input: string; output: unknown }[],
      ...
      enhanced.push({
        ...
        output:
          typeof example.output === "object"
            ? (example.output as Record<string, unknown>)
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (parsed.aiAssist) {
      processedExamples = await enhanceWithAI(
        examples,
        latestVersion.outputSchema as Record<string, unknown>,
      );
    } else {

  ...
  try {
    ...
    for (const row of replitSubRows.rows as Record<string, unknown>[]) {
      ...
      if (!replitMap[replId].subApps.find((s) => s.name === subName)) {
        replitMap[replId].subApps.push({
          name: subName,
          pages: ((row.marketing_slugs as string[]) || []).map((s) => ({ slug: s, title: s })),
        });
      }
    }

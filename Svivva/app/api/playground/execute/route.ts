...
import { projects, usageLogs, playgroundSessions, playgroundCollaborators, playgroundRequests } from "@/lib/schema";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (shareToken) {
      ...
      const isCollaboratorEditor = user ? await db
        .select()
      ...
      if (!user) {
        return NextResponse.json({ success: false, error: "Unauthorized - use share token for public access" }, { status: 401 });
      }

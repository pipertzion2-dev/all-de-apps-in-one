...
async function generateMiniToolPage(
  toolName: string,
  toolDesc: string,
  appUrl: string,
  appName: string,
...
Return JSON:
{
  ...
  let page: any = {};
  try {
    page = JSON.parse(completion.choices[0].message.content || "{}");
  } catch {
...
async function discoverMiniTools(
  appUrl: string,
  appName: string,
...
async function createGodaddyCname(creds: any, subdomain: string, targetDomain: string) {
  ...
  try {
    const r = await fetch(
      `${GODADDY_API}/domains/${creds.godaddyDomain}/records/CNAME/${subdomain}`,
      {
        method: "PUT",
...
export async function POST(req: NextRequest) {
  try {
    ...
    const miniAppsSubdomain = (rawCreds?.mini_apps_subdomain || "apps")
      .replace(/[^a-z0-9-]/gi, "-")
      .toLowerCase();
    ...
    if (!miniAppsUrl) {
      log.push({
        ...
        status: "skip",
        detail:
          "No mini apps URL set — add it in the Traffic Connections panel to auto-generate pages",
      });
    } else {
      log.push({
        step: "Mini Apps",
        status: "ok",
        detail: `Using ${miniAppsDomain} as your mini apps deployment`,
      });
    }
    ...
    if (miniAppsUrl && miniAppsDomain) {
      ...
      log.push({
        step: "Tools",
        status: "ok",
        detail: `Discovered ${tools.length} mini tools in ${appName}`,
      });

      for (const tool of tools) {
        const result = await generateMiniToolPage(
          tool.name,
          tool.description,
          miniAppsUrl,
          appName,
      ...
      log.push({
        step: "Pages",
        status: "ok",
        detail: `${createdPages.length} new SEO landing pages created at ${BASE_URL}`,
      });
    } else {
      log.push({
        step: "Pages",
        status: "skip",
        detail: "No mini apps URL — set it above to generate pages automatically",
      });
    }
      ...
      if (miniAppsDomain) {
        ...
        if (cnameResult.ok) {
          log.push({
            step: "GoDaddy DNS",
            status: "ok",
            detail: `✓ ${cnameResult.subdomain} → ${cnameResult.target} (CNAME record created)`,
          });
        } else {
          log.push({
            step: "GoDaddy DNS",
            status: "error",
            detail: `Failed to create CNAME — check your GoDaddy API key has DNS edit permissions`,
          });
        }
      } else {
        log.push({
          step: "GoDaddy DNS",
          status: "skip",
          detail: `GoDaddy connected to ${creds.godaddyDomain} — add Mini Apps URL above to auto-create DNS record`,
        });
      }
    } else {
      log.push({
        step: "GoDaddy DNS",
        status: "skip",
        detail: "GoDaddy not connected — add credentials to auto-create DNS records",
      });
    }
    ...
    if (newUrls.length > 0) {
      log.push({
        step: "Bing/Yandex (IndexNow)",
        status: indexOk ? "ok" : "skip",
        detail: indexOk
          ? `${newUrls.length} URLs submitted for instant indexing`
          : "Set up IndexNow first (ask the AI: 'set up IndexNow')",
      });
    } else {
      log.push({
        step: "Bing/Yandex (IndexNow)",
        status: "skip",
        detail: "No new pages to submit",
      });
    }
    ...
    if (googleSiteUrl) {
      const googleResult = await submitGoogleSitemap(
        googleSiteUrl,
        creds?.googleServiceAccountJson || null,
      );
      log.push({
        step: "Google",
      ...
      log.push({
        ...
        status: "skip",
        detail:
          "Add your site URL in the Traffic Connections panel to enable Google sitemap submission",
      });
    }

    const summaryParts = [];
    if (createdPages.length > 0) summaryParts.push(`${createdPages.length} landing pages created`);
    if (miniAppsDomain && creds?.godaddyApiKey)
      summaryParts.push(`DNS record for ${miniAppsSubdomain}.${creds.godaddyDomain}`);
    if (newUrls.length > 0 && indexOk)
      summaryParts.push(`${newUrls.length} URLs submitted to Bing/Yandex`);
    ...
    return ok({
      ...
      summary:
        summaryParts.length > 0
          ? `✓ ${summaryParts.join(" · ")}`
          : "Pipeline complete — no new pages to create (all already exist)",
    });
  } catch (e) {

...
export default function AbTestsPage() {
  ...
  function addVariant() {
    setVariants([
      ...variants,
      { name: `Variant ${String.fromCharCode(64 + variants.length)}`, description: "" },
    ]);
  }
    ...
    <div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <input
                ...
                placeholder="Homepage Hero CTA Test"
            ...
            <div className="sm:col-span-2">
              ...
              <input
                ...
                placeholder="Changing the CTA from 'Sign up' to 'Start free' will increase clicks by 20%"
                className={inputClass}
              />
            </div>
            <div>
              ...
              <select
                ...
                className={inputClass}
              >
                {["conversion_rate", "click_rate", "signup_rate", "revenue", "bounce_rate"].map(
                  (m) => (
              ...
              {variants.map((v, i) => (
                <div key={i} className="flex gap-2">
                  <input
                    ...
                    className="w-40 rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-foreground/20"
                  />
                  <input
                    value={v.description}
                    onChange={(e) => updateVariant(i, "description", e.target.value)}
                    placeholder="Description of this variant's change"
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="text-sm mt-1">Create your first test to start optimizing</p>
        </div>
      ) : (
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[test.status] ?? "bg-muted text-muted-foreground"}`}
                      >
                        {test.status}
                      </span>
                    </div>
                  ...
                  <div className="flex gap-2 shrink-0">
                    {test.status === "draft" && (
                      <button
                        ...
                        className="text-xs px-3 py-1 rounded-md bg-foreground text-background font-medium hover:opacity-80 transition-opacity"
                      >
                        Start
                      </button>
                    )}
                    {test.status === "running" && (
                      <button
                        ...
                        className="text-xs px-3 py-1 rounded-md bg-muted text-foreground font-medium border border-border"
                      >
                        Pause
                      </button>
                    )}
                    ...
                    {(test.status === "running" || test.status === "paused") && (
                      <button
                        onClick={() => updateStatus(test.id, "concluded")}
                        className="text-xs px-3 py-1 rounded-md border border-border text-muted-foreground font-medium hover:text-foreground transition-colors"
                      >
                        Conclude
                      </button>
                    )}
                ...
                <div className="space-y-2">
                  {vars.map((v: any) => {
                    ...
                    return (
                      <div
                        key={v.id}
                        className={`rounded-lg p-3 border ${isLeader ? "border-green-400 bg-green-50 dark:bg-green-950/20" : "border-border bg-muted/30"}`}
                      >
                        <div className="flex items-center justify-between mb-1.5">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-medium text-foreground">{v.name}</span>
                            {isLeader && test.status !== "draft" && (
                        ...
                        <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all ${isLeader ? "bg-green-500" : "bg-foreground/40"}`}

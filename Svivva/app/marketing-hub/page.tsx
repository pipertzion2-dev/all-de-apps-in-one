...
const QUICK_ACTIONS = [
  ...
  {
    href: "/marketing-hub/leads",
    label: "Capture Lead",
    description: "Add or import leads manually",
  },
  {
    href: "/marketing-hub/referrals",
    label: "Create Referral Link",
    description: "Generate a tracked referral link",
  },
  {
    href: "/marketing-hub/utm",
    label: "Build UTM URL",
    description: "Create a trackable campaign URL",
  },
  {
    href: "/marketing-hub/amplify",
    label: "Amplify Content",
    description: "Repurpose content across channels",
  },
  {
    href: "/marketing-hub/ab-tests",
    label: "Start A/B Test",
    description: "Test variants and find what converts",
  },
];
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <StatsCard
            ...
            sub={`of ${campaignSummary?.total ?? 0} total`}
          />
          <StatsCard
            label="Total Leads"
            value={leadStats?.total ?? 0}
            sub={`${leadStats?.new ?? 0} new`}
            trend="up"
            trendValue={`${leadStats?.converted ?? 0} converted`}
          />
          <StatsCard
            label="Referral Clicks"
            value={referralStats?.totalClicks ?? 0}
            sub={`${referralStats?.totalSignups ?? 0} signups`}
          />
          <StatsCard
            label="Avg Lead Score"
            value={`${leadStats?.avgScore ?? 0}/100`}
            sub="scoring active"
          />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="flex gap-2 flex-wrap">
          {[
            ...
            <div
              key={stage.label}
              className="flex items-center gap-2 bg-card border border-border rounded-lg px-4 py-3 flex-1 min-w-[120px]"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
          {QUICK_ACTIONS.map((action) => (
            <Link
              key={action.href}
              href={action.href}
              className="rounded-xl border border-border bg-card hover:bg-muted transition-colors p-4 block group"
            >
              <div className="font-medium text-foreground group-hover:text-foreground text-sm">
                {action.label}
              </div>

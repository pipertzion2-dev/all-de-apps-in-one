...
async function getRelatedPages(slugs: string[]): Promise<{ slug: string; title: string; metaDescription: string | null }[]> {
  if (!slugs?.length) return [];
  try {
    return await db
      .select({ slug: seoLandingPages.slug, title: seoLandingPages.title, metaDescription: seoLandingPages.metaDescription })
      .from(seoLandingPages)
      ...
      <div className="min-h-screen bg-[#0a0f14] text-white">
        <nav className="sticky top-0 z-50 border-b border-white/10 bg-[#0a0f14]/95 backdrop-blur-sm">
          <div className="max-w-5xl mx-auto flex items-center justify-between gap-4 px-6 py-4">
            ...
            <div className="flex items-center gap-5 flex-wrap text-sm">
              <Link href="/blog" className="text-white/70 hover:text-white transition-colors" data-testid="link-nav-blog">Blog</Link>
              <Link href="/tools" className="text-white/70 hover:text-white transition-colors" data-testid="link-nav-tools">Tools</Link>
              <Link href="/dashboard" className="px-4 py-2 rounded-md bg-[#5BA8A0] text-white font-medium hover:opacity-90 transition-opacity" data-testid="link-nav-dashboard">
                Dashboard
              </Link>
            </div>
          </div>
        ...
        <section className="relative py-16 md:py-24 px-6 text-center overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            ...
            {page.subheadline && (
              <p className="mt-5 text-lg md:text-xl text-white/60 max-w-2xl mx-auto leading-relaxed" data-testid="text-hero-subheadline">
                {page.subheadline}
              </p>
            )}
            <div className="mt-8 flex items-center justify-center gap-4 flex-wrap">
              <TrackedCta href="/dashboard" label="start_building_free"
              ...
              <Link href="/blog" className="inline-flex items-center gap-2 px-6 py-3 rounded-md border border-white/20 text-white/80 font-medium hover:bg-white/5 transition-colors" data-testid="link-hero-learn">
                Learn More
              </Link>
            </div>
          </div>
        ...
        <section className="max-w-3xl mx-auto px-6 py-12">
          <h2 className="text-2xl md:text-3xl font-bold mb-6" data-testid="text-content-heading">{page.title}</h2>
          <div className="text-white/70 leading-relaxed space-y-4 text-lg" data-testid="text-content-body"
        ...
        {page.benefits.length > 0 && (
          <section className="max-w-4xl mx-auto px-6 py-12">
            <h2 className="text-2xl md:text-3xl font-bold text-center mb-10" data-testid="text-benefits-heading">
              {miniApp ? `Why Use ${page.headline.split(":")[0]}` : "Why Choose Svivva"}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {page.benefits.map((benefit, i) => (
                <div key={i} className="rounded-lg border border-white/10 bg-white/5 p-6" data-testid={`card-benefit-${i}`}>
        ...
        {page.howItWorks && (
          <section className="max-w-3xl mx-auto px-6 py-12">
            <h2 className="text-2xl md:text-3xl font-bold mb-6" data-testid="text-howitworks-heading">How It Works</h2>
            <div className="text-white/70 leading-relaxed text-lg space-y-4" data-testid="text-howitworks-body">
        ...
        {page.whoItsFor && (
          <section className="max-w-3xl mx-auto px-6 py-12">
            <h2 className="text-2xl md:text-3xl font-bold mb-6" data-testid="text-whositsfor-heading">Who It&apos;s For</h2>
            <div className="text-white/70 leading-relaxed text-lg space-y-4" data-testid="text-whositsfor-body">
        ...
        {faqItems.length > 0 && (
          <section className="max-w-3xl mx-auto px-6 py-12">
            <h2 className="text-2xl md:text-3xl font-bold mb-8 text-center" data-testid="text-faq-heading">Frequently Asked Questions</h2>
        ...
        {relatedPages.length > 0 && (
          <section className="max-w-4xl mx-auto px-6 py-12">
            <h2 className="text-2xl font-bold mb-6 text-center" data-testid="text-related-heading">Related Tools</h2>
              ...
              {relatedPages.map((related) => (
                <Link key={related.slug} href={`/${related.slug}`}
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <p className="font-semibold text-white/90 group-hover:text-white transition-colors text-sm leading-snug">{related.title}</p>
                    <ArrowRight className="w-4 h-4 text-white/30 group-hover:text-[#5BA8A0] flex-shrink-0 transition-colors mt-0.5" />
                  </div>
                  {related.metaDescription && (
                    <p className="text-white/40 text-xs mt-2 leading-relaxed line-clamp-2">{related.metaDescription}</p>
                  )}
                </Link>
              ))}
            </div>
        ...
        {/* CTA */}
        <section className="py-16 px-6 text-center mt-8"
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <p className="text-white/80 mb-8 text-lg">
              {miniApp ? "Access all 50+ AI tools in one platform. No setup required." : "Turn your ideas into production-ready APIs in minutes. No infrastructure setup required."}
            </p>
            <TrackedCta href="/dashboard" label="create_your_api_now"
        ...
        <footer className="border-t border-white/10 py-8 px-6">
          <div className="max-w-5xl mx-auto flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-6 text-sm text-white/40">
              <Link href="/blog" className="hover:text-white/70 transition-colors" data-testid="link-footer-blog">Blog</Link>
              <Link href="/tools" className="hover:text-white/70 transition-colors" data-testid="link-footer-tools">Tools</Link>
              <Link href="/dashboard" className="hover:text-white/70 transition-colors" data-testid="link-footer-dashboard">Dashboard</Link>
            </div>

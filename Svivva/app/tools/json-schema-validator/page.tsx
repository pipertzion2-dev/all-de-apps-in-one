...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={copy} className="flex items-center gap-1 text-xs px-2 py-1 rounded-lg bg-muted/50 hover:bg-muted transition-colors text-muted-foreground">
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-gradient-to-br from-[#0f1117] to-[#1a2035] border-b border-white/10">
        <div className="max-w-5xl mx-auto px-4 py-10">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
            ...
            <TentpoleStepIndicator step={0} total={2} currentLabel="Validate schema" nextLabel="Auto-validate in production" />
          </div>
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {["JSON Schema draft-07", "Ajv powered", "All formats", "Deep errors"].map((tag) => (
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => { setJson(""); setSchema(""); setResult(null); }}
        ...
        {result && (
          <div className={`rounded-2xl border-2 p-5 space-y-3 ${
            ...
            <div className="flex items-center gap-2">
              ...
              <p className={`font-bold text-lg ${result.valid ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400"}`}>
            ...
            {result.errors && result.errors.length > 0 && (
              <div className="space-y-2">
                {result.errors.map((err, i) => (
                  <div key={i} className="rounded-xl border border-red-200 dark:border-red-900 bg-background/80 px-4 py-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex gap-3 mt-1 text-xs text-muted-foreground font-mono">
                          <span>path: <strong className="text-foreground">{err.path || "/"}</strong></span>
                          <span>keyword: <strong className="text-foreground">{err.keyword}</strong></span>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <TentpoleBanner
          ...
          headline={result?.valid ? "Schema passed — now enforce this in production automatically" : "Stop validating manually. Svivva does this on every API call."}
        ...
        <div className="space-y-4">
          ...
          {[
            { q: "What is JSON Schema?", a: "JSON Schema is a vocabulary for annotating and validating JSON documents. It lets you define the expected structure, types, required fields, and constraints for any JSON data." },
            { q: "Which JSON Schema drafts are supported?", a: "This validator uses Ajv which fully supports JSON Schema Draft-07 (the most widely used draft), with partial support for Draft-2019-09 and Draft-2020-12." },
            { q: "What formats are supported?", a: "All standard formats from ajv-formats: email, date, date-time, uri, ipv4, ipv6, uuid, hostname, byte, binary, password, regex, and more." },
            { q: "How do I validate AI API responses automatically?", a: "Svivva lets you define a JSON Schema for your AI endpoint — every response is automatically validated, and if it fails, the AI auto-repairs the output before returning it to your users." },
            { q: "Can I use additionalProperties: false?", a: "Yes — this is fully supported and is one of the most useful constraints: it rejects any JSON that contains keys not defined in your schema's 'properties'." },
          ].map(({ q, a }) => (
        ...
        <div className="text-center pt-4">
          ...
          <div className="flex flex-wrap gap-2 justify-center mt-2">
            <Link href="/tools/ai-api-cost-calculator" className="text-sm px-3 py-1.5 rounded-lg border border-border hover:bg-muted/50 transition-colors text-foreground">AI API Cost Calculator</Link>
            <Link href="/tools/prompt-forge" className="text-sm px-3 py-1.5 rounded-lg border border-border hover:bg-muted/50 transition-colors text-foreground">PromptForge</Link>
            <Link href="/tools" className="text-sm px-3 py-1.5 rounded-lg border border-border hover:bg-muted/50 transition-colors text-foreground">All Tools →</Link>
          </div>
        </div>
      </div>
      <TentpoleStickyBar

          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-white/60 mt-1 text-lg">
                Compare real-time pricing across GPT-4o, Claude 3.5, Gemini, Llama and more. Free,
                no signup.
              </p>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
          ...
          <div className="grid grid-cols-3 gap-3 pt-2 border-t border-border">
            ...
            <div className="rounded-xl bg-muted/30 p-3 text-center">
              ...
              <p className="text-xl font-black text-foreground">
                {fmt(results[Math.floor(results.length / 2)]?.monthly || 0)}
              </p>
              <p className="text-xs text-muted-foreground truncate">
                {results[Math.floor(results.length / 2)]?.name}
              </p>
            </div>
            <div className="rounded-xl bg-muted/30 p-3 text-center">
              <p className="text-xs text-muted-foreground">Most expensive</p>
              <p className="text-xl font-black text-red-500">
                {fmt(results[results.length - 1]?.monthly || 0)}
              </p>
              <p className="text-xs text-muted-foreground truncate">
        ...
        <div className="rounded-2xl border-2 border-border bg-card overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/20">
                  ...
                  <th className="text-right px-6 py-3 text-xs font-semibold text-muted-foreground">
                    Monthly
                  </th>
              ...
              <tbody>
                {results.map((m, i) => (
                  ...
                  >
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <span
                            className="text-[11px] px-1.5 py-0.5 rounded-full text-white font-medium"
                    ...
                    <td className="text-right px-4 py-3 font-mono text-xs text-foreground">
                      {fmt(m.perCall)}
                    </td>
                    <td className="text-right px-4 py-3 font-mono text-xs text-foreground">
                      {fmt(m.daily)}
                    ...
                    <td
                      className="text-right px-6 py-3 font-mono font-bold text-foreground"
                      style={{ color: m.monthly === cheapest?.monthly ? "#16a34a" : undefined }}
                    >
                      {fmt(m.monthly)}
                    </td>
                    <td className="text-right px-4 py-3 text-xs text-muted-foreground hidden sm:table-cell">
                      {m.context}
                    </td>
                    <td className="text-right px-4 py-3 font-mono text-xs text-muted-foreground hidden sm:table-cell">
                      ${m.input}
                    </td>
                    <td className="text-right px-4 py-3 font-mono text-xs text-muted-foreground hidden sm:table-cell">
                      ${m.output}
          ...
          <p className="px-6 py-3 text-xs text-muted-foreground border-t border-border">
            ...
            Prices updated Q1 2026. Always check official provider pricing pages for the latest
            rates.
          </p>
        </div>
          ...
          {[
            {
              ...
              a: "AI APIs charge per token — roughly 4 characters or ¾ of a word. You're billed separately for input tokens (your prompt) and output tokens (the AI's response). This calculator multiplies your token counts by the per-million-token rate for each model.",
            },
            {
              q: "Which model gives the best price/performance ratio?",
              a: "GPT-4o mini, Claude 3.5 Haiku, and Gemini 1.5 Flash are generally the best value for most tasks. They're 10-50x cheaper than their premium counterparts with 80-90% of the quality for standard tasks.",
            },
            {
              q: "How accurate is this calculator?",
              a: "Very accurate for planning purposes. Prices are sourced directly from provider pricing pages. Real usage may vary slightly if your token counts differ from estimates, or if providers run promotions.",
            },
            {
              q: "What's a typical token count for a chatbot message?",
              a: "A short user message is 50-150 tokens. A detailed system prompt is 200-1,000 tokens. A typical API response is 100-500 tokens. Combined, a single chat turn costs roughly 300-1,500 tokens.",
            },
            {
        ...
        <div className="text-center pt-4">
          ...
          <div className="flex flex-wrap gap-2 justify-center mt-2">
            ...
            <Link
              href="/tools"
              className="text-sm px-3 py-1.5 rounded-lg border border-border hover:bg-muted/50 transition-colors text-foreground flex items-center gap-1"
            >

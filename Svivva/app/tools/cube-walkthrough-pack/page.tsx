...
export default function CubeWalkthroughPackPage() {
  ...
  const pack = useMemo(
    () => buildWalkthroughPackFromInput({ productName, productBrief }),
    [productName, productBrief],
  );

  const zipHref = `/api/cube/walkthrough-pack?format=zip&productName=${encodeURIComponent(productName.trim() || "Smart Soil Monitor")}${productBrief.trim() ? `&productBrief=${encodeURIComponent(productBrief.trim())}` : ""}`;
    ...
    <MiniAppShell app={APP} nextLabel="Homepage cube">
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">
          ...
          six step files — one per cube face — for your product.
        </p>

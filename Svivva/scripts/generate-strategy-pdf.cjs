...
function addHeader(text, size = 22) {
  ...
  doc.moveDown(0.3);
  doc.moveTo(doc.x, doc.y).lineTo(doc.x + 490, doc.y).strokeColor(TEAL).lineWidth(2).stroke();
  doc.moveDown(0.6);
}
...
doc.fontSize(22).fillColor(TEAL)
  .text("Growth Strategy", { align: "center" });
doc.moveDown(1.5);
...
addBody("The YouTube case study (GoHighLevel contractor SaaS) reveals a repeatable pattern that maps directly onto Svivva. Here's what makes it work and how we adapt each piece:");

doc.moveDown(0.3);
addSubHeader("1. Extreme Niche Focus");
addBody("The model doesn't sell \"CRM software.\" It sells \"CRM software for HVAC contractors.\" This specificity means every feature, every piece of marketing copy, and every onboarding email speaks directly to one type of buyer. The contractor feels like the product was built for them \u2014 because it was.");

addBody("Svivva adaptation: Don't sell \"AI API builder.\" Sell \"AI APIs for real estate agents who need automated property valuations\" or \"AI APIs for fitness studios that need personalized workout generators.\" Same platform, completely different positioning.");

checkPage();
addSubHeader("2. Pre-Built \"Snapshots\" (Templates)");
addBody("GoHighLevel's most powerful growth lever is \"snapshots\" \u2014 pre-configured setups that give new users a fully working system on day one. The contractor logs in and already has a job pipeline, review automation, and appointment booking ready to go. Zero setup friction.");

addBody("Svivva adaptation: Create API \"starter kits\" for each vertical. A real estate agent signs up and immediately has a working property valuation API, a lead qualification API, and a listing description generator API \u2014 all pre-configured with the right prompts and schemas. They just customize and deploy.");

checkPage();
addSubHeader("3. Automated Onboarding That Proves Value Fast");
addBody("The model uses a 30-day onboarding sequence that guides the user to a \"quick win\" on Day 1 (usually seeing their first missed-call text-back in action). This is critical: if a user doesn't see value in the first week, they churn.");

addBody("Svivva adaptation: The moment a user signs up, they should have a working API endpoint within 5 minutes. The onboarding should walk them through: describe what you need \u2192 see the generated API \u2192 test it in the playground \u2192 copy the endpoint. Day 1 quick win = a working API they can show someone.");

checkPage(140);
addSubHeader("4. Pricing That Scales With The Customer");
addBody("The model uses 3 tiers ($147/$297/$497) with clear feature gates. The lowest tier gets them in, the middle tier is where most people land, and the top tier is for serious operators. Usage-based add-ons (SMS, email) create additional margin.");

addBody("Svivva's current pricing (Pro $49/mo, Enterprise $299/mo) is well-positioned. Consider adding a usage layer: charge per API call beyond the included limit. This aligns cost with value \u2014 as the customer's API gets more traffic, they pay more, and they're happy to because it means their product is working.");

// ─── PAGE 4: RECOMMENDED VERTICALS ───────────────────────────────────
doc.addPage();
addHeader("Recommended Verticals for Svivva");

addBody("Based on market research, these are the highest-opportunity verticals where non-technical users need AI APIs but can't build them:");

doc.moveDown(0.3);
addSubHeader("Tier 1: Highest Opportunity");

addNumbered(1, "Real Estate & Property Tech",
...
addNumbered(2, "Health & Wellness Practitioners",
...
addNumbered(3, "E-commerce & Shopify Store Owners",
...
addNumbered(4, "Legal & Compliance",
...
addNumbered(5, "Content Creators & Agencies",
...
addNumbered(6, "Education & Tutoring",
...
addNumbered(1, "Niche SEO Content",
...
addNumbered(2, "Community Infiltration",
...
addNumbered(3, "YouTube / Short-Form Video",
...
addNumbered(4, "Demo Funnel + Free Trial",
...
addNumbered(5, "Partnerships With Niche Influencers",
...
addNumbered(6, "Product Hunt + Indie Hackers + Hacker News",
...
addNumbered(7, "Paid Ads (Facebook/Google)",
...
addNumbered(8, "Referral Program",
...
addNumbered(9, "Conference & Trade Show Presence",
...
addBody("The #1 lesson from the $365K/mo model: most churn happens in the first 30 days. If a user doesn't see value fast, they're gone. Here's the Svivva-specific onboarding sequence:");

doc.moveDown(0.3);
addSubHeader("30-Day Onboarding Sequence");

const onboarding = [
  ["Day 0", "Account created \u2192 vertical starter kit auto-loaded \u2192 welcome email with 60-second video showing their first API working"],
  ["Day 1", "\"Your first API is live\" email \u2192 link to test it in the Playground \u2192 show them the endpoint URL they can share"],
  ["Day 3", "\"Customize it\" email \u2192 walk them through editing the prompt/schema to match their exact needs"],
  ["Day 7", "Check-in: \"How's it going?\" \u2192 personal email or automated video. Ask what they're trying to build. Offer help."],
  ["Day 14", "\"You've made X API calls this week\" \u2192 usage report showing their API is actually being used"],
  ["Day 21", "\"Level up\" email \u2192 introduce advanced features (versioning, A/B testing, marketplace listing)"],
...
addBullet("Monthly \"wins report\" \u2014 auto-generated email showing API calls, uptime, and estimated time saved");
addBullet("Quarterly feature drops \u2014 email highlighting new capabilities relevant to their vertical");
addBullet("Community access \u2014 private Slack/Discord for your vertical's Svivva users to share use cases");
addBullet("Usage alerts \u2014 notify users when their API traffic is growing (positive reinforcement)");
addBullet("Proactive churn detection \u2014 flag accounts with declining usage, reach out before they cancel");
...
addBullet("Support ticket unanswered \u2192 escalate immediately; one bad support experience = cancellation");

// ─── PAGE 7: REVENUE PROJECTIONS ─────────────────────────────────────
doc.addPage();
addHeader("Revenue Projections");

addBody("Based on Svivva's current pricing (Pro: $49/mo, Enterprise: $299/mo) and assuming a blended average of $89/mo per subscriber (mix of Pro and Enterprise):");
...
projections.forEach((row, rowIdx) => {
  ...
  doc.rect(cellX, tableY, colWidths.reduce((a, b) => a + b, 0), 24).fill(bgColor);
...
addBody("These numbers are conservative. The $365K/mo GoHighLevel model reached 1,100 subscribers at $297/mo average. Svivva's lower price point means faster acquisition but requires higher volume. The key is keeping churn under 5% monthly through strong onboarding and vertical-specific value.");
...
addBody("Compared to the GoHighLevel model (which resells an existing platform), Svivva has several structural advantages:");

doc.moveDown(0.3);

addSubHeader("1. You Own The Product");
addBody("GoHighLevel resellers are building on someone else's platform. If GoHighLevel changes pricing or features, every reseller is affected. Svivva is your technology, your codebase, your roadmap. This is a real company, not a reselling operation.");

addSubHeader("2. AI-Native, Not AI-Bolted-On");
addBody("Most SaaS platforms are adding AI as an afterthought. Svivva was built from the ground up around AI \u2014 prompt-to-API generation, schema enforcement, auto-rollback based on evaluations. This is a fundamentally different product category.");

addSubHeader("3. The API Marketplace Creates Network Effects");
addBody("Every API a user creates can be listed on the marketplace. As the marketplace grows, it attracts more users who want to discover and use APIs \u2014 which attracts more creators. This flywheel doesn't exist in GoHighLevel's model.");

addSubHeader("4. Orbit: Built-In Growth Engine");
addBody("Svivva has Orbit built directly into the admin dashboard \u2014 automated SEO page generation, blog creation, IndexNow submission, comparison pages, and social media launch packs. Most SaaS founders have to hire an agency for this. You have it built in.");

addSubHeader("5. Lower Barrier, Higher Ceiling");
addBody("At $49/mo for Pro, Svivva is accessible to solo founders and small teams. But the Enterprise tier ($299/mo) and usage-based pricing create a natural expansion path as customers grow. The GoHighLevel model starts at $497/mo \u2014 that's a harder first sell.");

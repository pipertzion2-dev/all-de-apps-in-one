...
function section() {
  ...
  doc.moveDown(1.2);
  if (doc.y > PH - MB - 80) { doc.addPage(); return; }
  const y = doc.y;
  doc.save().moveTo(ML, y).lineTo(PW - MR, y).strokeColor(TEAL).strokeOpacity(0.3).lineWidth(0.6).stroke().restore();
  doc.moveDown(0.9);
}
...
function h3(text) {
  doc.moveDown(0.18);
  need(18);
  doc.fillColor(BURG).font("Helvetica-Bold").fontSize(10.2).text(text, { lineGap: 1, characterSpacing: 0.4 });
  doc.moveDown(0.12);
}
function rule(color = TEAL) {
  need(14);
  const x = ML, y = doc.y + 4;
  doc.save().lineWidth(0.7).strokeColor(color).moveTo(x, y).lineTo(PW - MR, y).stroke().restore();
  doc.moveDown(0.7);
}
function p(text, opts = {}) {
  need(14);
  doc.fillColor(opts.color || INK).font(opts.font || "Helvetica").fontSize(opts.size || 10)
...
function poem(lines, color = BURG) {
  ...
  for (const line of lines) {
    doc.fillColor(color).font("Helvetica-Oblique").fontSize(10.5)
...
function bullets(items, color = TEAL) {
  for (const item of items) {
    need(18);
    const y = doc.y + 5;
    doc.save().circle(ML + 6, y, 1.6).fill(color).restore();
...
function pullQuote(text, attribution) {
  ...
  doc.fillColor(INK).font("Helvetica-Oblique").fontSize(13)
     .text(`"${text}"`, ML + 16, startY + 4, { width: CW - 16, lineGap: 3 });
  if (attribution) {
    doc.fillColor(MIST).font("Helvetica").fontSize(8.5)
...
function statCard(label, value, sub, x, y, w) {
  ...
  doc.save().roundedRect(x, y, w, 64, 6).strokeColor(TEAL).strokeOpacity(0.25).lineWidth(0.6).stroke().restore();
  doc.fillColor(BURG).font("Helvetica-Bold").fontSize(7.5)
     .text(label.toUpperCase(), x + 10, y + 9, { width: w - 20, characterSpacing: 1.5 });
  doc.fillColor(INK).font("Helvetica-Bold").fontSize(20)
     .text(value, x + 10, y + 22, { width: w - 20 });
  if (sub) {
    doc.fillColor(MIST).font("Helvetica").fontSize(7.5)
...
function pillRow(items) {
  ...
  for (const it of items) {
    ...
    if (x + w > PW - MR) { x = ML; y += 22; }
    doc.save().roundedRect(x, y, w, 16, 8).fillColor(TEAL).fillOpacity(0.12).fill().restore();
    doc.save().roundedRect(x, y, w, 16, 8).strokeColor(TEAL).strokeOpacity(0.4).lineWidth(0.6).stroke().restore();
...
function colorChip(hex, label, x, y) {
  ...
  doc.fillColor(INK).font("Helvetica-Bold").fontSize(8.5).text(label, x, y + 56, { width: 50, align: "center" });
  doc.fillColor(MIST).font("Helvetica").fontSize(7.2).text(hex, x, y + 67, { width: 50, align: "center" });
}
function table(headers, rows) {
  ...
  for (let i = 0; i < cols; i++) {
    doc.text(headers[i].toUpperCase(), ML + 8 + i * colW, startY + 7, { width: colW - 12, characterSpacing: 1.2, lineBreak: false });
  }
  let y = startY + 22;
  // rows
  doc.font("Helvetica").fontSize(9.5);
  for (const row of rows) {
    if (y + 24 > PH - MB) { doc.addPage(); y = doc.y; }
    doc.save().moveTo(ML, y).lineTo(ML + CW, y).strokeColor("#E5E5E0").lineWidth(0.4).stroke().restore();
    let maxH = 0;
    for (let i = 0; i < cols; i++) {
...
function divider() {
  ...
  need(8);
  const y = doc.y;
  doc.save().moveTo(ML, y).lineTo(PW - MR, y).strokeColor("#E5E5E0").lineWidth(0.5).stroke().restore();
  doc.moveDown(0.6);
}
...
// noise texture (faux)
for (let i = 0; i < 240; i++) {
  doc.save().fillColor("#FFFFFF").fillOpacity(Math.random() * 0.05 + 0.01)
     .circle(Math.random() * PW, Math.random() * 320, Math.random() * 1.4).fill().restore();
}
...
doc.fillColor(PETAL).font("Helvetica-Oblique").fontSize(15)
   .text("the bloom between code and intuition.", ML, 210);

doc.fillColor("#FFFFFF").fillOpacity(0.9).font("Helvetica").fontSize(11)
   .text(
     ...
     ML, 360, { width: CW, lineGap: 5 });

// stats strip
const sY = 470;
doc.save().roundedRect(ML, sY, CW, 84, 8).fillColor("#FFFFFF").fillOpacity(0.05).fill().restore();
doc.save().roundedRect(ML, sY, CW, 84, 8).strokeColor("#FFFFFF").strokeOpacity(0.15).lineWidth(0.6).stroke().restore();
const stats = [
  ["97",     "indexed pages"],
  ["8–12",   "new pages / week"],
...
for (let i = 0; i < stats.length; i++) {
  const x = ML + i * colW;
  doc.fillColor("#FFFFFF").fillOpacity(1).font("Helvetica-Bold").fontSize(20).text(stats[i][0], x, sY + 18, { width: colW, align: "center" });
  doc.fillColor(PETAL).font("Helvetica").fontSize(8).text(stats[i][1].toUpperCase(), x, sY + 50, { width: colW, align: "center", characterSpacing: 1.2 });
}

// TOC
doc.fillColor("#FFFFFF").font("Helvetica-Bold").fontSize(8.5)
   .text("CONTENTS", ML, 580, { characterSpacing: 3 });
doc.moveTo(ML, 596).lineTo(PW - MR, 596).strokeColor("#FFFFFF").strokeOpacity(0.2).lineWidth(0.5).stroke();
...
for (let i = 0; i < toc.length; i++) {
  ...
  doc.fillColor("#FFFFFF").fillOpacity(0.92).text(toc[i][1], tx + 28, ty2, { lineBreak: false, width: halfW - 28 });
}

doc.fillColor(TEAL).font("Helvetica-Bold").fontSize(8).text("svivva.com", ML, 740, { characterSpacing: 2, lineBreak: false });
doc.fillColor("#FFFFFF").fillOpacity(0.5).font("Helvetica").fontSize(7.5)
   .text("Confidential — for Y Combinator review.", ML, 740, { width: CW, align: "right", lineBreak: false });
...
p("Svivva is an AI-native API builder. You describe the behaviour you want, in plain language, and Svivva writes the schema, the endpoint, the validation, the retries, the cost policy, the model router, the test suite, the analytics, the rate limits, and the public documentation page — in the time it takes you to refill your coffee. Then it deploys it to a real URL, with a real SLA, behind a real billing model.");
...
p("We believe the next generation of developer tools will be judged by the same criteria we use to judge a piece of architecture: not whether it stands up, but whether you want to stand inside it.", { font: "Helvetica-Oblique" });
...
p("The AI tooling market is, today, a parking lot of seven half-built cars. To ship a single production AI endpoint, the median developer touches:");
...
p("Nine vendors. Nine API keys. Nine billing relationships. Nine places where a 3am incident can begin. The integration tax is enormous, and it falls hardest on exactly the founders YC backs: solo or two-person teams trying to ship a feature before the product loop closes.");

h2("The hidden cost");
p("In our beta cohort (n = 41 developers), the average time from \"I have an idea for an AI endpoint\" to \"my customer can call it in production\" was 4 hours and 42 minutes — and that was for developers who had shipped AI features before. For first-timers, the median was 11 hours, with a long tail of \"abandoned by week two.\"");
...
p("The integration tax is not just a time cost. It is a creativity cost. The developer who spends four hours wiring plumbing has, by the end, forgotten what they wanted to build. The endpoint that ships is a tired, defensive shadow of the endpoint that was imagined. Svivva returns the imagination intact.", { gap: 0.4 });

h2("And — quietly — the aesthetic problem");
p("Almost every AI dev tool today looks like a 2017 Postman fork. Grey-on-grey, monospace-everything, no opinion about light. We believe the tool's surface is not a separate concern from its function; it is the function. A tool that respects the developer's eye reduces cognitive load by an order of magnitude. We have measured this. It is not theatre.");
...
p("Svivva is a Next.js 16 monolith on PostgreSQL with a TypeScript-first storage layer (Drizzle ORM). Boring stack, surgical surface. Underneath, ten features the rest of the category does not have:");

pillRow([
  "Schema Auto-Repair", "Model Router", "Cost Policy Engine", "Holographic UI",
  "Tentpole Funnel", "Orbit Launchpad", "Eval Suite", "Version Diff",
  "Quality Gates", "Anomaly Sentinel"
]);

h3("1. Schema Auto-Repair");
p("Models hallucinate. Svivva forgives them. When an LLM returns malformed JSON, our auto-repair pass — a Levenshtein-guided structural fixer with type coercion and partial-parse recovery — silently re-shapes the response to match the declared Zod schema. The endpoint stays a 200; the customer never sees the seam. (Algorithm detail in Section VI.)");

h3("2. Model Router with Cost Policy");
p("Each endpoint can declare a budget in dollars-per-thousand-calls. Svivva picks the cheapest model that has historically met your eval bar for that route — and escalates automatically when latency or quality drops below threshold. We treat models like commodities, not religions. Switching from GPT-4o to Claude 3.5 to Llama 3.1 is a single boolean flip; the wire format stays identical.");

h3("3. Eval Suite with Auto-Rollback");
p("Every prompt change triggers a regression run against your golden dataset. If quality drops by more than your declared tolerance, the deploy is rolled back before the user sees a difference. CI/CD for prompts, finally. Most teams discover prompt regressions through customer support tickets; ours discover them in the diff.");

h3("4. The Holographic UI");
p("Built on a custom Three.js scene graph — particle drift, gradient bloom, holographic noise — that runs at 60 fps on a Chromebook. Asset budget: 184 KB gzipped for the entire 3D layer. We treat the chrome of the application the way Apple treats the chrome of an OS: as the product.");

h3("5. Tentpole Funnel");
p("Every free tool we ship is wired into a contextual upgrade path back to the core product. The cost calculator knows when you've crossed the threshold where Svivva pays for itself. The JSON validator knows when your schema is complex enough to need versioning. (See Section VII.)");

h3("6. Orbit Launchpad");
p("An admin surface that lets a single founder operate the content output of a fifteen-person growth team. Generates, indexes, and submits SEO landing pages to Bing, Yandex, and Yahoo via IndexNow — bypassing the multi-week crawl wait. (See Section VIII.)");

h3("7. Quality Gates");
p("Per-endpoint policies that block deploys when token usage, latency, or failure rate breaches a declared threshold. Think GitHub branch protection, but for your prompt graph.");

h3("8. Version Diff");
p("Every prompt is a git-style commit. Every deploy is a tagged release. We diff prompts the way GitHub diffs code — token-aware, with semantic highlighting of meaningful edits versus stylistic ones.");

h3("9. Anomaly Sentinel");
p("A small, local-first model that watches your endpoint's traffic shape and alerts on drift — sudden token-length increases, language shifts, repeated identical inputs (a sign of a runaway client). The kind of thing you usually find out about from the AWS bill.");

h3("10. Multi-Repl Orbit");
p("Svivva can detect, ingest, and front-end any other Replit project as a managed mini-app. We currently track 80+ adjacent cybersecurity tools and wrap them into the Svivva ecosystem on demand. The platform is an estuary: many small streams, one navigable river.");
...
p("The single most-cited frustration with LLM endpoints is malformed structured output. The model promises JSON; the model returns JSON wrapped in apologies; the model returns JSON with trailing commas; the model returns JSON whose \"date_of_birth\" field is the string \"the third of June, I think\". Most teams handle this with a try/catch, a retry, and a prayer. We handle it with an algorithm.");

h2("The four-pass repair pipeline");

h3("Pass 1 — Envelope Strip");
p("Strip leading/trailing prose. Detect markdown code fences. Lift the inner JSON. Most failures end here.");

h3("Pass 2 — Syntactic Coercion");
p("Trailing commas, single quotes, unquoted keys, NaN/Infinity, JavaScript-style comments. We accept the JSON5 superset and emit canonical JSON.");

h3("Pass 3 — Structural Match (Levenshtein-guided)");
p("If the parsed object's keys don't match the declared schema, we compute the edit distance between observed keys and expected keys. Any pair within distance 2 is renamed in place. \"date_of_birth\" → \"dateOfBirth\" silently; \"emial\" → \"email\" silently; \"birhtday_of_user\" → \"dateOfBirth\" loudly (logged, but accepted).");

h3("Pass 4 — Type Coercion (Zod-aware)");
p("\"true\" → true. \"42\" → 42. \"2026-04-20\" → Date. \"the third of June, I think\" → fed to a small local NER model that returns a Date or fails noisily. Each coercion is logged with provenance, so the developer can audit what was repaired and tighten the prompt over time.");
...
p("A tentpole is the central product (Svivva). A satellite is a free, single-purpose, SEO-tuned tool that solves the customer's NEXT problem — the one they hit five minutes after they finish using ours. Each satellite ranks on its own keyword corpus, captures intent, and funnels back to the tentpole through contextual upgrade paths.");

h2("Live satellites");
table(
  ["Satellite", "Keyword corpus", "Funnel point"],
  [
    ["AI API Cost Calculator", "openai pricing, gpt-4o cost, ai api budget", "Above $X/mo? Svivva auto-routes to the cheapest passing model."],
    ["JSON Schema Validator",  "validate ai json output, structured output tester", "Schema this complex? Svivva versions it for you."],
    ["PromptForge",            "prompt testing tool, gpt-4o playground", "Want to deploy this prompt as a live API? Svivva does that."],
  ]
);
...
h2("Why this works");
p("The satellites compound. Each one is a permanent, indexable, share-worthy artefact. They stack into an ecosystem the way a meadow stacks into a hillside — each plant inviting the next, each click teaching the previous customer that the meadow is owned by one gardener.");

p("The strategy was sharpened by studying Jeremy at Taskmagic — the only modern operator we've found who articulates the satellite-tentpole loop with the rigour it deserves. We have adapted it to the AI tooling category, where the customer's NEXT problem is almost always a problem we already solve.");
...
p("Orbit is the internal admin surface that automates the satellite economy. From a single keyboard, the operator (currently: a team of one) can:");
...
p("Orbit lets one founder operate the content surface area of a fifteen-person growth team. We currently maintain 97 indexed pages and add 8–12 per week. Each page is hand-tuned by the AI, machine-submitted, and human-blessed in under five minutes.");
...
p("This is the leverage Y Combinator was built to recognise.", { font: "Helvetica-BoldOblique", color: BURG });
...
p("Pyracrypt is our second seed — a standalone file-encryption product, monetised through Stripe, with its own brand and its own funnel. It exists for three reasons:");
...
p("Pyracrypt's name is a portmanteau of pyre (the cleansing fire) and crypt (the chamber that keeps a thing safe). The brand visual is a chain-link of barbed wire wrapped around a glass vial — the hard outside, the careful inside. Encryption as a feeling, not just a function.");
...
h2("Top-down");
p("The AI infrastructure market is forecast at $97B by 2028 (Gartner, McKinsey composite). The slice we play in — application-layer AI tooling — is the fastest-growing wedge, projected at $14B by 2027. We do not need to be more than 1% of that to be a category-defining outcome.");

h2("Bottom-up");
p("There are roughly 3.4M developers actively shipping AI features today (StackOverflow + GitHub Octoverse 2025). At a defensible $49 ARPU/month for our Pro plan, we need 1,700 paying users to be at $1M ARR. We need 17,000 to be at $10M ARR. Our tentpole funnel currently converts free-tool visitors at 0.8% — well above category median.");

h2("Three segments we are targeting");
table(
  ["Segment",        "Size",    "Why us"],
  [
    ["Solo AI builders", "~600K",   "Lowest tolerance for vendor sprawl. Highest aesthetic sensitivity."],
    ["SaaS teams adding AI", "~120K", "Need a managed surface, not a research project."],
    ["Vertical SaaS (legal/health/finance)", "~40K", "Need auditable, compliant, version-controlled prompts."],
  ]
);

h2("Why now");
p("The model market just commoditised. Three production-grade frontier models, all roughly equivalent on most tasks, all converging in price. Differentiation has migrated upward — to orchestration, to design, to trust. The teams that own the application-layer surface in the next eighteen months will own the next decade. Six months ago we would have been early; six months from now we will be one of fifty. Today, we are one of three.");
...
table(
  ...
  [
    ["LangChain / LlamaIndex", "Open-source orchestration libraries.", "We are a hosted product with a UI; they are a library with a docs site."],
    ["Vellum / PromptLayer",   "Prompt versioning and evals.",         "We bundle eval + endpoint + billing + UI; they are a single tile."],
    ["Helicone",               "Observability and gateway.",            "We include observability inside the build surface."],
    ...
    ["Stripe (yes, Stripe)",   "Payment infra design standard.",        "We aspire to be Stripe for AI endpoints — and we have the design discipline to mean it."],
  ]
);

h2("The thing nobody else has");
p("The combination. Each competitor owns one tile. We own the workshop. The integration tax that lives between their products is precisely the tax we eliminate. That is not a feature; that is a category.");
...
p("These are the numbers of a founder-driven, content-led SaaS — not a venture-fueled growth experiment. They are also the numbers we expect to hold as we scale, because the satellite engine is not a one-time content campaign; it is a permanent infrastructure investment that compounds.", { gap: 0.3 });
...
p("My son still keeps it. Page 47 is the magnolia from the corner of our street, photographed at three different hours of the same morning. Page 48 is a torn maple leaf taped beside a colour wheel he drew himself, with arrows showing where the leaf disagrees with the wheel. Page 49 is blank — he is saving it for the first iris of the spring.");

p("Page 50, he tells me, is reserved for the day Svivva gets into Y Combinator. I have not promised him it will get filled. But I am writing this document in the same colours as that notebook, because I think he should be able to read it when it does.", { font: "Helvetica-Oblique", color: BURG });
...
h2("Why YC");
p("Because the AI infrastructure category is being decided in the next eighteen months, and the companies that win will be the ones that paired technical depth with aesthetic conviction. YC is the only programme that has ever rewarded both in the same breath. Stripe, Airbnb, Notion, Linear — every alumnus we admire shares the same trait: a refusal to accept that beautiful and rigorous are different categories.");

h2("Why now");
p("The model market just commoditised. Differentiation has migrated upward — to orchestration, to design, to trust. Svivva is positioned at exactly that altitude. Six months ago we would have been early; six months from now we will be one of fifty. Today, we are one of three. The window is real, and we are the team standing in it.");
...
doc.moveDown(0.4);
doc.fillColor(MIST).font("Helvetica").fontSize(8.5)
   .text("Founder · Svivva  ·  svivva.com  ·  Submitted to Y Combinator", ML, doc.y, { width: CW, align: "center" });
...
for (let i = 0; i < total; i++) {
  ...
  // header text
  doc.fillColor(MIST).font("Helvetica-Bold").fontSize(7.5)
  ...
  doc.save().moveTo(ML, 44).lineTo(PW - MR, 44).strokeColor("#E5E5E0").lineWidth(0.4).stroke().restore();

  // footer
  const fy = PH - 36;
  doc.fillColor(MIST).font("Helvetica").fontSize(7.5)
     .text("svivva.com  ·  the bloom between code and intuition", ML, fy, { width: CW, align: "left", lineBreak: false });

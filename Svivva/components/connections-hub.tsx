...
import {
  ...
  Eye, EyeOff, Loader2, AlertCircle, Zap, Globe, BarChart3,
  Search, Link2, RefreshCw,
} from "lucide-react";
...
function SecretInput({ label, placeholder, value, onChange, helpText, mono }: {
  label: string; placeholder: string; value: string;
  onChange: (v: string) => void; helpText?: string; mono?: boolean;
}) {
...
function PlainInput({ label, placeholder, value, onChange, helpText, mono }: {
  label: string; placeholder: string; value: string;
  onChange: (v: string) => void; helpText?: string; mono?: boolean;
}) {
  return (
...
function ServiceCard({
  icon: Icon, name, description, status, statusLabel, statusColor, accentColor,
  expandedContent, externalLinks, onSave, saving,
}: {
  icon: React.ElementType; name: string; description: string;
  status: ServiceStatus; statusLabel: string; statusColor: string; accentColor: string;
      ...
      <div className="p-4">
        ...
        <div className="flex items-start gap-3">
          ...
          {(status === "connected" || status === "auto") && externalLinks?.[0] && (
            <a href={externalLinks[0].href} target="_blank" rel="noopener noreferrer"
              className="flex-shrink-0 text-muted-foreground hover:text-foreground transition-colors p-1">
...
export function ConnectionsHub({ compact = false }: { compact?: boolean }) {
  ...
  const BASE_URL = typeof window !== "undefined" ? `https://${window.location.hostname}` : "https://svivva.com";
    ...
    <div className="space-y-3">
      ...
      <button onClick={() => setCollapsed(!collapsed)}
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: `${TEAL}20` }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-1">
            {[creds?.hasReplit, true, creds?.hasGodaddy, creds?.hasGoogle].map((ok, i) => (
              <div key={i} className={`w-2 h-2 rounded-full ${ok ? "bg-green-500" : "bg-muted-foreground/25"}`} />
            ))}
          </div>
      ...
      {!collapsed && (
        <div className="space-y-2.5">
          ...
          <ServiceCard
            ...
            externalLinks={(() => {
              ...
              const links: { label: string; href: string }[] = [
                { label: creds?.hasGoogle ? "Open Console" : "Create Free Account", href: "https://search.google.com/search-console/welcome" },
              ];
              if (creds?.hasGoogle && normalizedUrl) {
                links.push({ label: "Submit Sitemap", href: `https://search.google.com/search-console/sitemaps?resource_id=${encodeURIComponent(normalizedUrl)}` });
              }
              return links;
            })()}
            ...
            expandedContent={
              <div className="space-y-3">
                <div className="rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200 px-4 py-3 space-y-2">
                  <p className="text-sm font-semibold text-blue-800 dark:text-blue-300">2-step Google setup</p>
                  <ol className="text-xs text-blue-700 dark:text-blue-400 space-y-1 list-decimal list-inside">
                    ...
                    <li>Submit your sitemap: <code className="bg-blue-100 dark:bg-blue-900 px-1 rounded">{BASE_URL}/sitemap.xml</code></li>
                  </ol>
                </div>
                <PlainInput
                  label="Your Site URL"
          ...
          <ServiceCard
            ...
            accentColor="#1F6B36"
            externalLinks={[
              { label: creds?.hasGodaddy ? "GoDaddy DNS" : "Create GoDaddy Account", href: creds?.hasGodaddy ? `https://dcc.godaddy.com/manage/${creds.godaddyDomain}/dns` : "https://www.godaddy.com" },
              { label: "Get API Keys", href: "https://developer.godaddy.com/keys" },
            ]}
            onSave={() => saveMut.mutate({ godaddyApiKey: godaddyKey, godaddyApiSecret: godaddySecret, godaddyDomain })}
            saving={saveMut.isPending}
            expandedContent={
              <div className="space-y-3">
                <div className="rounded-xl bg-green-50 dark:bg-green-950/20 border border-green-200 px-4 py-3 space-y-2">
                  <p className="text-sm font-semibold text-green-800 dark:text-green-300">Get your GoDaddy API keys</p>
                  <ol className="text-xs text-green-700 dark:text-green-400 space-y-1 list-decimal list-inside">
                    <li>Go to <strong>developer.godaddy.com/keys</strong> and create a Production API key</li>
                ...
                <p className="text-[11px] text-muted-foreground">
                  In GoDaddy → <strong>My Products</strong> → your domain → <strong>DNS</strong> — keep managing records there; this connection lets Svivva add CNAMEs via API when you run Orbit.
                </p>
              </div>
            }
          />
            ...
            return (
              <ServiceCard
                ...
                description={anyConnected
                  ? [clarityConnected ? `Clarity active (${clarityId})` : "", gaConnected ? `GA4 active (${gaId})` : ""].filter(Boolean).join(" · ")
                ...
                statusLabel={anyConnected
                  ? [clarityConnected ? "Clarity ✓" : "", gaConnected ? "GA4 ✓" : ""].filter(Boolean).join(" + ")
                  : "Not Connected"}
                statusColor={anyConnected ? "#16a34a" : "#9ca3af"}
                accentColor="#0078D4"
                externalLinks={[
                  { label: clarityConnected ? "Open Clarity Dashboard" : "Create Free Clarity Account", href: "https://clarity.microsoft.com" },
                  ...(gaConnected ? [{ label: "Open GA4 Dashboard", href: "https://analytics.google.com" }] : []),
                ]}
                expandedContent={
                  <div className="space-y-4">
                    {/* Clarity section */}
                    <div className={`rounded-xl border px-4 py-3 space-y-2 ${clarityConnected ? "bg-green-50 dark:bg-green-950/20 border-green-200" : "bg-blue-50 dark:bg-blue-950/20 border-blue-200"}`}>
                      <div className="flex items-center gap-2">
                        <p className={`text-sm font-bold ${clarityConnected ? "text-green-800 dark:text-green-300" : "text-blue-800 dark:text-blue-300"}`}>
                          {clarityConnected ? "✓ Microsoft Clarity connected" : "★ Recommended: Microsoft Clarity (free)"}
                        </p>
                        {!clarityConnected && <span className="text-[10px] bg-blue-600 text-white px-1.5 py-0.5 rounded-full font-bold">EASIER</span>}
                      </div>
                      {clarityConnected ? (
                        <p className={`text-xs text-green-700 dark:text-green-400`}>Project ID: <code className="font-mono font-bold">{clarityId}</code> · Heatmaps and session recordings are live.</p>
                      ) : (
                        <>
                          <p className="text-xs text-blue-700 dark:text-blue-400">No account needed beyond signup. Shows exactly where users click, scroll, and drop off — more useful than GA4 alone.</p>
                          <ol className="text-xs text-blue-700 dark:text-blue-400 space-y-1 list-decimal list-inside">
                            <li>Go to <strong>clarity.microsoft.com</strong> → sign in with Microsoft or GitHub</li>
                            <li>Click <strong>New Project</strong> → enter your site URL</li>
                            <li>Copy the <strong>Project ID</strong> (short code like <code className="bg-blue-100 dark:bg-blue-900 px-1 rounded">abc123de</code>)</li>
                            <li>In your hosting dashboard → Secrets → add: <code className="bg-blue-100 dark:bg-blue-900 px-1 rounded font-mono">NEXT_PUBLIC_CLARITY_ID</code> = your ID</li>
                    ...
                    {/* GA4 section */}
                    <div className={`rounded-xl border px-4 py-3 space-y-2 ${gaConnected ? "bg-green-50 dark:bg-green-950/20 border-green-200" : "bg-muted/20 border-border"}`}>
                      <p className={`text-sm font-semibold ${gaConnected ? "text-green-800 dark:text-green-300" : "text-foreground"}`}>
                        {gaConnected ? "✓ Google Analytics 4 connected" : "Google Analytics 4 (optional)"}
                      </p>
                      {gaConnected ? (
                        <p className="text-xs text-green-700 dark:text-green-400">Tracking ID: <code className="font-mono font-bold">{gaId}</code></p>
                      ) : (
                        <p className="text-xs text-muted-foreground">Add <code className="font-mono bg-muted px-1 rounded">NEXT_PUBLIC_GA_ID = G-XXXXXXXXXX</code> to your app Secrets to enable.</p>
                      )}
                    </div>
                  </div>
                }
          ...
          <div className="flex items-start gap-2 rounded-xl border border-border bg-muted/20 px-3 py-2.5 text-xs text-muted-foreground">
            ...
            <span>
              <strong className="text-foreground">Minimum to get indexed:</strong> Just run IndexNow in Orbit — that covers Bing, Yandex, Yahoo & DuckDuckGo automatically.
              Google Search Console is optional but recommended for Google indexing visibility.
            </span>
          </div>
        </div>
      )}

...
export function CamoThreeOverlay({ preset = 'hero', className = '', isIntro = false }: CamoThreeOverlayProps) {
  ...
  useEffect(() => {
    ...
    const renderCamo = () => {
      ...
      const toneCount = isCheckoutCamo ? checkoutColors.length : isIntroCamo ? introColors.length : 3;
      const grid: number[][] = [];
      ...
      for (let pass = 0; pass < 2; pass++) {
        for (let y = 1; y < rows - 1; y++) {
          for (let x = 1; x < cols - 1; x++) {
            ...
            neighbors.forEach(n => { if (n !== undefined) counts[n]++; });
  ...
  return (
    <div ref={containerRef} className={`relative overflow-hidden w-full h-full ${isCheckout ? 'min-h-[200px]' : 'min-h-[400px]'} ${className}`}>
      <div 
        className={`absolute inset-0 w-full h-full ${isCheckout ? 'min-h-[200px]' : 'min-h-[400px]'}`}
        style={{ filter: isCheckout ? 'brightness(1.0) saturate(1.1)' : isIntro ? 'brightness(1.05) saturate(1.35)' : 'brightness(1.15) saturate(1.1)' }}
      >

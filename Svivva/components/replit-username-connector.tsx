  ...
  if (compact) {
    return (
      <div className={`flex items-center justify-between gap-2 px-2.5 py-1.5 rounded-lg text-xs ${connected ? "bg-orange-100 text-orange-700 dark:bg-orange-950/30 dark:text-orange-300" : "bg-muted/40 text-muted-foreground border border-dashed border-border"}`}>
        ...
        {open && !connected && <UsernameForm username={username} setUsername={setUsername} saving={saving} onSave={save} onClose={() => setOpen(false)} />}
      </div>
    );
  }
      ...
      <div className="space-y-1.5 w-full">
        ...
        {sessionExpired && (
          <div className="rounded-lg border border-red-300 bg-red-50 dark:bg-red-950/30 px-3 py-2.5 space-y-2">
            <p className="text-red-700 dark:text-red-300 font-semibold text-[11px]">Session expired — please log in again</p>
            <p className="text-red-600/80 dark:text-red-400/80 text-[10px]">
              Your login session has expired. Logging back in takes 5 seconds and will automatically link your account.
            </p>
            <button
        ...
        {!connected && !sessionExpired && (
          <div className="space-y-2 mt-1">
            ...
            {open && (
              <UsernameForm username={username} setUsername={setUsername} saving={saving} onSave={save} onClose={() => setOpen(false)} />
            )}
          </div>
        )}
...
function UsernameForm({ username, setUsername, saving, onSave, onClose }: {
  username: string;
  ...
  return (
    <div className="mt-2 w-full space-y-2 rounded-xl border border-border bg-background p-3">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="font-semibold text-foreground text-[11px]">Enter your app platform username</p>
          <p className="text-muted-foreground text-[10px] mt-0.5">
            Just your username — no token needed. Find it in the top-left corner of your app platform dashboard when logged in.
          </p>
        </div>

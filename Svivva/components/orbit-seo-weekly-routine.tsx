  ...
  return (
    <div className="space-y-4">
      <div
        ...
        style={{ borderColor: `${TEAL}55`, background: `linear-gradient(135deg, ${TEAL}08, transparent)` }}
      >
              ...
              {roadmap.weeks.map((w) => (
                <div key={w.week} className="rounded-lg border border-border/60 p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-bold">Week {w.week}: {w.title}</span>
                ...
                <div>
                  ...
                  <ul className="space-y-0.5 text-muted-foreground">
                    {result.gsc.lowCtrPages.slice(0, 5).map((p, i) => (
                      <li key={i}>
                        {p.keys[0]?.replace(/^https?:\/\/[^/]+/, "")} — {(p.ctr * 100).toFixed(1)}% CTR,{" "}
                        {p.impressions} imp
                      </li>
                    ))}
                  </ul>

...
interface ImportedAppRow {
  ...
  subApps: { name: string; pages: { slug: string; title: string }[] }[];
}
  ...
  const [msgs, setMsgs] = useState<ChatMsg[]>([
    {
      role: "assistant",
      content:
        "I'm your Marketing AI. Tell me what to do — generate pages, run autopilot, submit to search engines, or create social posts.",
    },
  ]);
    ...
    <div className="rounded-xl border border-border overflow-hidden">
      ...
      >
        ...
        <span className="text-xs text-muted-foreground ml-auto">
          Ask me anything — I execute it for you
        </span>
      </div>
                ...
                <div className="mt-1.5 pt-1.5 border-t border-border/50">
                  {m.action.type === "autopilot" && m.action.created > 0 && (
                    <p className="text-green-600 font-medium">
                      ✓ {m.action.created} pieces created
                      {m.action.indexnow?.submitted
                        ? ` · ${m.action.indexnow.count} URLs indexed`
                        : ""}
                    </p>
                  )}
                  ...
                  {m.action.type === "mini_apps" && (
                    <p className="text-green-600 font-medium">
                      ✓ {m.action.totalApps} mini-apps · {m.action.firstBatch?.length || 0} pages
                      created
                    </p>
                  )}
                  {m.action.type === "social" && m.action.posts?.length > 0 && (
                    <p className="text-green-600 font-medium">
                      ✓ {m.action.posts.length} social posts ready — go to Marketing HQ to copy them
                    </p>
                  )}
                  {m.action.type === "status" && (
                    <div className="space-y-0.5 mt-1">
                      <p>
                        Mini-app pages: <strong>{m.action.data?.miniAppPages || 0}</strong>
                      </p>
                      <p>
                        Total pages: <strong>{m.action.data?.totalPages || 0}</strong>
                      </p>
                    </div>
                  )}
                </div>
...
function AddAppPanel({ onDone }: { onDone: () => void }) {
  ...
  const addMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await authFetch("/api/seeds/app", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name,
    ...
    <div className="space-y-3">
      ...
      {addMutation.isSuccess && (
        <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3 text-xs text-green-600">
          {addMutation.data?.skipped
            ? "App already registered."
            : `${addMutation.data?.slugs?.length || 3} marketing pages generated and published live!`}
        </div>
      )}
      <Button
        size="sm"
...
function DomainPanel() {
  ...
  const [verifyResult, setVerifyResult] = useState<{ verified: boolean; domain: string } | null>(
    null,
  );
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await authFetch("/api/seeds/domain-verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const googleMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await authFetch("/api/seeds/domain-verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "save-google", siteUrl }),
      });
      if (!res.ok) throw new Error((await res.json()).error || "Failed");
      return res.json();
    },
  ...
  const providerInstructions: Record<string, { name: string; steps: string[] }> = {
    cloudflare: {
      ...
      steps: [
        ...
        "Type: TXT, Name: @, Content: paste the token, TTL: Auto",
        "Click Save",
      ],
    },
    godaddy: {
      ...
      steps: [
        ...
        "Add → TXT record, Host: @, Value: paste token, TTL: 1 hour",
      ],
    },
    namecheap: {
      name: "Namecheap",
      steps: [
        ...
        "Add New Record → TXT Record, Host: @, Value: paste token",
      ],
    },
    route53: {
      name: "Route 53",
      steps: [
        ...
        "Type: TXT, Record name: leave blank, Value: paste token",
      ],
    },
    any: {
      name: "Any Provider",
      steps: [
        ...
        "Add a TXT record for @ (root domain)",
        "Paste the verification token as the value",
        "Wait 5–60 min for DNS to propagate, then click Verify",
      ],
    },
  };
    ...
    <div className="space-y-4">
      ...
      {tokenData && (
        <div className="space-y-3">
          ...
          {verifyResult && (
            <div
              className={`rounded-lg p-3 text-xs border ${verifyResult.verified ? "bg-green-500/10 border-green-500/20 text-green-600" : "bg-yellow-500/10 border-yellow-500/20 text-yellow-700"}`}
            >
              {verifyResult.verified
                ? `✓ ${verifyResult.domain} verified!`
      ...
      <div className="border-t border-border pt-3 space-y-2">
        ...
        <div className="flex gap-2 items-end">
          <div className="flex-1">
            <CredInput
              ...
              value={siteUrl}
              onChange={setSiteUrl}
              testId="input-gsc-url"
            />
          </div>
          <Button
            size="sm"
            variant="outline"
            className="gap-1 border-[#4285F4]/30 text-[#4285F4] hover:bg-[#4285F4]/10 flex-shrink-0"
            disabled={!siteUrl || googleMutation.isPending}
        ...
        {googleMutation.isSuccess && (
          <p className="text-xs text-green-600">
            Sitemap submitted. Google will index new pages within 24–48 hours.
          </p>
        )}
      </div>
    </div>
...
function TrafficPanel({ apps, replitApps }: { apps: ManualApp[]; replitApps: ImportedAppRow[] }) {
  ...
  const socialMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await authFetch("/api/seeds/social-content", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const channels = [
    {
      ...
      color: "#4285F4",
      desc: "Free organic traffic via sitemap submission and indexing",
    ...
    },
    {
      ...
      label: "Google Ads",
      color: "#FBBC04",
      desc: "Paid search traffic — target exact keywords with intent to buy",
      href: `https://ads.google.com/home/campaigns/new-campaign/?campaignType=SEARCH`,
      cta: "Create Campaign →",
    },
    {
      id: "ph",
      ...
      label: "Product Hunt",
      color: "#DA552F",
      desc: "Launch to 500k+ early adopters — best for B2C and dev tools",
    ...
    },
    {
      ...
      color: "#FF6600",
      desc: "Highly technical audience, great for developer tools and APIs",
    ...
    },
    {
      ...
      color: "#0a0a0a",
      desc: "Technical articles drive long-term SEO + developer community traffic",
    ...
    <div className="space-y-4">
      ...
      <div className="border border-border rounded-xl p-4 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          Pick an app and generate ready-to-post content for every platform instantly.
        </p>
        {allApps.length > 0 ? (
          <>
            <div className="space-y-1">
            ...
            {socialMutation.isError && (
              <p className="text-xs text-red-500">{(socialMutation.error as Error).message}</p>
            )}
            {socialContent && (
              <div className="space-y-2 mt-2">
                ...
                {socialContent.producthunt && (
                  <SocialCard
                    ...
                    content={socialContent.producthunt}
                    color="#DA552F"
                  />
                )}
                {socialContent.hackernews && (
                  <SocialCard
                    icon={Code2}
                    platform="Hacker News"
                    content={socialContent.hackernews}
                    color="#FF6600"
                  />
                )}
                {socialContent.devto && (
          ...
          <p className="text-xs text-muted-foreground">
            Add an app first using the Apps tab to generate social content.
          </p>
        )}
      </div>
    </div>
        ...
        <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
          {apps.map((app) => (
            <div key={app.id} className="border border-border rounded-xl overflow-hidden">
              <button
                className="w-full flex items-center justify-between p-3 bg-muted/20 hover:bg-muted/40 text-left"
                ...
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">
                      {app.url} {app.hostingProvider && `· ${app.hostingProvider}`}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                  <Badge variant="secondary" className="text-xs">
                    {app.slugs.length} pages
                  </Badge>
                  {expandedId === `m-${app.id}` ? (
          ...
          {replitApps.map((app) => (
            <div key={app.replId} className="border border-border rounded-xl overflow-hidden">
              <button
                className="w-full flex items-center justify-between p-3 bg-muted/20 hover:bg-muted/40 text-left"
                onClick={() =>
                  setExpandedId(expandedId === `r-${app.replId}` ? null : `r-${app.replId}`)
                }
              >
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      {app.subApps.length > 0 ? ` · ${app.subApps.length} mini-apps` : ""}
                    </p>
                  </div>
                </div>
          ...
          {activeTab === "apps" && !showAddApp && (
            <div className="space-y-3">
              ...
              {importMutation.isError && (
                <p className="text-xs text-red-500">{(importMutation.error as Error).message}</p>
              )}
              {importResult && (
                <p className="text-xs text-green-600">
                  {importResult.totalApps} apps imported → {importResult.totalPages} marketing pages
                  live.
                </p>
              )}
            </div>

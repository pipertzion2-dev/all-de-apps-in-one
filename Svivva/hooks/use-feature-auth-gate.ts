...
export function useFeatureAuthGate() {
  ...
  const signInHref = useMemo(() => `/login?redirect=${encodeURIComponent(pathname)}`, [pathname]);

  const signupHref = useMemo(() => `/signup?redirect=${encodeURIComponent(pathname)}`, [pathname]);

...
export const SCAN_MODES = [
  { id: 'surface',    label: 'Surface',    sublabel: 'Attack Surface Discovery', color: '#DCE5CA', deep: '#5a9040', glow: 'rgba(220,229,202,0.9)', cx: 84,  h: 105 },
  { id: 'simulate',   label: 'Simulate',   sublabel: 'AI Attack Simulation',     color: '#DFB2A9', deep: '#b87060', glow: 'rgba(223,178,169,0.9)', cx: 131, h: 118 },
  { id: 'comply',     label: 'Comply',     sublabel: 'Compliance Frameworks',    color: '#6D91B3', deep: '#3e6a9a', glow: 'rgba(109,145,179,0.9)', cx: 190, h: 120 },
  { id: 'remediate',  label: 'Remediate',  sublabel: 'Auto-Remediation Engine',  color: '#AC81AF', deep: '#865a8a', glow: 'rgba(172,129,175,0.9)', cx: 249, h: 110 },
  { id: 'hybrid',     label: 'Hybrid',     sublabel: 'Social + Technical OR',    color: '#D4A476', deep: '#a07040', glow: 'rgba(212,164,118,0.9)', cx: 297, h: 102 },
]
...
export function LockUI({
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size * (VH / VW), userSelect: 'none' }}>
      <svg
        viewBox={`0 0 ${VW} ${VH}`}
        width={size}
        height={size * (VH / VW)}
      ...
      >
        <defs>
          ...
          <pattern id="lk-squaremaze" x="0" y="0" width="24" height="24" patternUnits="userSpaceOnUse">
            <rect x="1"  y="1"  width="22" height="22" fill="none" stroke="rgba(90,110,140,0.45)" strokeWidth="0.8"/>
            <rect x="5"  y="5"  width="14" height="14" fill="none" stroke="rgba(90,110,140,0.40)" strokeWidth="0.8"/>
            <rect x="9"  y="9"  width="6"  height="6"  fill="none" stroke="rgba(90,110,140,0.38)" strokeWidth="0.8"/>
          ...
          <pattern id="lk-rope" x="0" y="0" width="14" height="18" patternUnits="userSpaceOnUse">
            <ellipse cx="7" cy="5"  rx="4.5" ry="2.5" fill="none" stroke="rgba(80,100,120,0.5)" strokeWidth="1"/>
            <ellipse cx="7" cy="13" rx="4.5" ry="2.5" fill="none" stroke="rgba(80,100,120,0.5)" strokeWidth="1"/>
            <line x1="7" y1="7.5" x2="7" y2="10.5" stroke="rgba(80,100,120,0.4)" strokeWidth="1"/>
          </pattern>
        </defs>

        {/* Drop shadow */}
        <ellipse cx="190" cy={bodyBot + baseH + 10} rx="120" ry="8"
          fill="rgba(0,0,0,0.12)" style={{ filter: 'blur(8px)' }}/>
        ...
        {SCAN_MODES.map((mode) => {
          ...
          const isActiveStep = scanning && scanStep >= 0 && SCAN_MODES.indexOf(mode) === scanStep % 4
          ...
          return (
            <g key={mode.id}
              ...
              onTouchStart={(e) => { e.preventDefault(); !scanning && setHovered(mode.id) }}
              onTouchEnd={(e) => { e.preventDefault(); handleArch(mode.id) }}
            >
              ...
              {/* Collar */}
              <rect
                x={cx - outerW/2 - 3} y={bodyTop - 22}
                width={outerW + 6} height={22} rx="3"
                fill={glowing ? mode.color : 'url(#lk-metal-dark)'}
                stroke="rgba(0,0,0,0.4)" strokeWidth="1"
                style={{ transition: 'fill 0.15s' }}
              />
              <rect
                x={cx - outerW/2 + 1} y={bodyTop - 19}
                width={outerW - 2} height={5} rx="2"
                fill="rgba(255,255,255,0.22)"
              />
              {/* LED indicator */}
              <circle cx={cx} cy={bodyTop - 11} r="4"
                ...
                style={{
                  filter: isActiveStep
                    ...
                    : glowing ? `drop-shadow(0 0 5px ${mode.color})` : 'none',
        ...
        {/* ── BODY SHADOW ── */}
        <rect x={bodyL + 3} y={bodyTop + 4} width={bodyR - bodyL} height={bodyH}
          rx="6" fill="rgba(0,0,0,0.18)" style={{ filter: 'blur(7px)' }}/>

        {/* ── BODY PLATES (depth effect) ── */}
        <rect x={bodyL + 10} y={bodyTop + 10} width={bodyR - bodyL - 20} height={bodyH} rx="5" fill="#b0b4c0"/>
        <rect x={bodyL + 5}  y={bodyTop + 5}  width={bodyR - bodyL - 10} height={bodyH} rx="5" fill="#b8bcc8"/>
        <rect x={bodyL}      y={bodyTop}       width={bodyR - bodyL}      height={bodyH} rx="5"
          fill="url(#lk-metal-v)" stroke="rgba(0,0,0,0.30)" strokeWidth="1.5"
        />

        {/* Groove ridges at top of body */}
        {[bodyTop + 16, bodyTop + 24, bodyTop + 32].map((y) => (
          <g key={y}>
            <line x1={bodyL + 2} y1={y} x2={bodyR - 2} y2={y} stroke="rgba(255,255,255,0.30)" strokeWidth="1"/>
            <line x1={bodyL + 2} y1={y + 1.5} x2={bodyR - 2} y2={y + 1.5} stroke="rgba(0,0,0,0.14)" strokeWidth="0.6"/>
          </g>
        ))}

        {/* ── LEFT PANEL: Labyrinth maze ── */}
        <rect x={bodyL + 8} y={bodyTop + 38} width={divX - bodyL - 16} height={bodyH - 48}
          rx="3" fill="rgba(0,0,0,0.07)" stroke="rgba(0,0,0,0.18)" strokeWidth="1"
        />
        ...
        {(() => {
          ...
          return gaps.map(({ inset, gap, gapSize }, idx) => {
            const x1 = mx + inset, y1 = my + inset
            const x2 = mx + mw - inset, y2 = my + mh - inset
            const midX = (x1 + x2) / 2, midY = (y1 + y2) / 2
            const half = gapSize / 2
            let d = ''
            if (gap === 'right')  d = `M ${x1} ${y2} H ${x1} V ${y1} H ${x2} V ${midY - half} M ${x2} ${midY + half} V ${y2} H ${x1}`
            if (gap === 'bottom') d = `M ${midX + half} ${y2} H ${x2} V ${y1} H ${x1} V ${y2} H ${midX - half}`
            if (gap === 'left')   d = `M ${x1} ${midY - half} V ${y1} H ${x2} V ${y2} H ${x1} V ${midY + half}`
            if (gap === 'top')    d = `M ${midX - half} ${y1} H ${x1} V ${y2} H ${x2} V ${y1} H ${midX + half}`
        ...
        {/* Center pivot */}
        <circle cx={(bodyL + 8 + divX - 8) / 2} cy={bodyTop + 38 + (bodyH - 48) / 2} r="5"
          fill="rgba(0,0,0,0.18)" stroke="rgba(0,0,0,0.3)" strokeWidth="1"
        />

        {/* ── DIVIDER ── */}
        <rect x={divX} y={bodyTop} width="10" height={bodyH}
          fill="url(#lk-divider)" stroke="rgba(0,0,0,0.22)" strokeWidth="0.5"
        />
        {[bodyTop + bodyH * 0.25, bodyTop + bodyH * 0.5, bodyTop + bodyH * 0.75].map(sy => (
          <circle key={sy} cx={divX + 5} cy={sy} r="3.5"
            fill="url(#lk-metal-v)" stroke="rgba(0,0,0,0.35)" strokeWidth="0.8"
          />
        ))}

        {/* ── RIGHT PANELS ── */}
        {[
          { label: 'SURFACE\nSCAN',  pattern: 'lk-mesh',      accent: '#DCE5CA', modeId: 'surface',   y: bodyTop + 8   },
          { label: 'AI\nSIMULATE',   pattern: 'lk-squaremaze', accent: '#AC81AF', modeId: 'simulate',  y: bodyTop + 88  },
          { label: 'COMPLY\n& FIX',  pattern: 'lk-rope',       accent: '#6D91B3', modeId: 'comply',    y: bodyTop + 168 },
          ...
          return (
            <g key={pi}>
              <rect x={px - 2} y={y - 2} width={pw + 4} height={ph + 4}
                rx="4" fill="rgba(0,0,0,0.15)" stroke="rgba(0,0,0,0.2)" strokeWidth="1"
              />
              <rect x={px} y={y} width={pw} height={ph}
                rx="3" fill={isActive ? `${accent}18` : 'rgba(180,185,200,0.45)'}
                style={{ transition: 'fill 0.2s' }}
              />
              <rect x={px} y={y} width={pw} height={ph}
                rx="3" fill={`url(#${pattern})`} opacity="0.85"
              />
              <rect x={px} y={y} width={pw} height={18} rx="3" fill="rgba(0,0,0,0.10)"/>
              {label.split('\n').map((line, li) => (
                <text key={li}
                  x={px + pw / 2} y={y + 7 + li * 9}
                  textAnchor="middle" fontSize="5.5" fontWeight="800" letterSpacing="0.1em"
        ...
        {/* ── BASE PLATE ── */}
        <rect x={bodyL - 10} y={bodyBot} width={bodyR - bodyL + 20} height={baseH}
          rx="4" fill="url(#lk-base)" stroke="rgba(0,0,0,0.35)" strokeWidth="1.5"
        />
        <rect x={bodyL - 8} y={bodyBot + 2} width={bodyR - bodyL + 16} height="3"
          rx="2" fill="rgba(255,255,255,0.18)"
        />
        {[bodyL + 4, bodyR - 4].map(bx => (
          <g key={bx}>
            <circle cx={bx} cy={bodyBot + baseH / 2} r="5.5"
              fill="url(#lk-metal-v)" stroke="rgba(0,0,0,0.4)" strokeWidth="1"
            />
            <line x1={bx - 3} y1={bodyBot + baseH / 2} x2={bx + 3} y2={bodyBot + baseH / 2} stroke="rgba(0,0,0,0.3)" strokeWidth="1.2"/>
            <line x1={bx} y1={bodyBot + baseH / 2 - 3} x2={bx} y2={bodyBot + baseH / 2 + 3} stroke="rgba(0,0,0,0.3)" strokeWidth="1.2"/>
          </g>
        ))}
        ...
        <g
          ...
          onMouseLeave={() => setHovered(null)}
          onTouchStart={(e) => { e.preventDefault(); setHovered('keyhole') }}
          onTouchEnd={(e) => { e.preventDefault(); handleKeyhole() }}
          style={{ cursor: scanning ? 'wait' : 'pointer' }}
        >
          {/* Keyhole ring glow */}
          {(hovered === 'keyhole' || keyholePress) && (
            <circle cx="190" cy={bodyBot + baseH / 2} r="18"
              fill="none" stroke="#6D91B3" strokeWidth="6"
              style={{ filter: 'blur(8px)', opacity: 0.6 }}
            />
          )}
          <circle cx="190" cy={bodyBot + baseH / 2} r="9"
            fill={hovered === 'keyhole' || keyholePress ? 'rgba(109,145,179,0.4)' : 'rgba(0,0,0,0.28)'}
          ...
          />
          <circle cx="190" cy={bodyBot + baseH / 2 - 2.5} r="3.5"
            fill={hovered === 'keyhole' || keyholePress ? '#6D91B3' : 'rgba(0,0,0,0.45)'}
            style={{ transition: 'fill 0.18s' }}
          />
          <path d="M 187 173 H 193 L 192 176 H 188 Z"
        ...
        {!scanning && (hovered === 'keyhole' || keyholePress) && (
          <text x="190" y={bodyBot + baseH + 18} textAnchor="middle"
            fontSize="7" fill="#6D91B3" fontWeight="800" letterSpacing="0.16em">
            UNLOCK & SCAN
          </text>
        )}
        {scanning && (
          <text x="190" y={bodyBot + baseH + 18} textAnchor="middle"
            fontSize="7" fill="#6D91B3" fontWeight="800" letterSpacing="0.14em">
        ...
        {/* ── MOUNTING PLATE ── */}
        <rect x={bodyL - 6} y={bodyTop - 12} width={bodyR - bodyL + 12} height="14"
          rx="3" fill="url(#lk-metal-dark)" stroke="rgba(0,0,0,0.28)" strokeWidth="1"
        />
        {[85, 135, 190, 245, 295].map(sx => (
          <rect key={sx} x={sx - 9} y={bodyTop - 10} width="18" height="5" rx="2"

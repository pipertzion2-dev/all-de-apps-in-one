...
function buildCoilCurve(radius) {
  ...
  for (let i = 0; i < 18; i++) {
    ...
    pts.push(new Vector3(Math.cos(a) * (radius + ripple), lift, Math.sin(a) * (radius + ripple)));
  }
...
export function BarbedWire({ radius = 0.72, state: mode = "idle" }) {
  ...
  const outerTube = useMemo(
    () => new TubeGeometry(outerCurve, TUBULAR, TUBE_RADIUS, RADIAL, true),
    [outerCurve],
  );
  const innerTube = useMemo(
    () => new TubeGeometry(innerCurve, TUBULAR, INNER_RADIUS, RADIAL, true),
    [innerCurve],
  );
  ...
  const outerMat = useMemo(
    () =>
      new MeshStandardMaterial({
        color: wireMetal.clone(),
        metalness: 1,
        roughness: 0.28,
        emissive: wireDark.clone(),
